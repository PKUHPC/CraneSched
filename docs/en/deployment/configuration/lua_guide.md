...
host does not need a Lua development package. Builds configured with
`-DCRANE_USE_SYSTEM_LUA=ON` use the system Lua development package instead.
The bundled runtime is Lua 5.4.8; scripts that rely on a different Lua ABI
should use the system-Lua mode.
...
- `Lua not found` during CMake configuration: this is expected only with
  `-DCRANE_USE_SYSTEM_LUA=ON`; install the matching platform Lua development
  package, or build with `-DCRANE_ENABLE_LUA=OFF`.

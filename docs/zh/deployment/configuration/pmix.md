...
|组件| 最低版本 | 说明|
|-----|------|-----|
OpenPMIx | 3.x | PMIx 运行时库（必须）|
Open MPI | 4.1+ | 支持 PMIx 的 MPI 实现（推荐 4.1.6）|
...
以 Open MPI 4.1.6 + 系统 PMIx 为例。建议参考OpenMpi官方文档安装
```bash
## 推荐安装在共享目录，如/nfs
...
```bash
# 管理员启用方式：
cmake -DWITH_PMIX=/nfs/software/pmix/3.1.7（pmix安装目录） [其他选项] ..

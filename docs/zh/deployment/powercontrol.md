...
#### 节能数据训练模型
部署在ctld节点。
拉取仓库: <https://github.com/PKUHPC/CraneSched-powersaving-trainning>
参考<https://github.com/PKUHPC/CraneSched-powersaving-trainning/blob/main/README.md>安装并启动predictor
...
systemctl enable craned  ## 必须开启自启动，否则调度宿主机开机后，计算节点无法自启动并恢复组网
```
#### craned相关插件服务
详细参考**[插件指南](./frontend/plugins.md)**
...
systemctl enable cplugind   ## 必须开启自启动，否则调度宿主机开机后，计算节点的插件无法自启动
```

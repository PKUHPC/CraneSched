...
当运行多任务crun作业（使用 `--ntasks-per-node > 1`）时，如果任何任务失败，crun将接收作业中所有任务的详细退出码信息。这有助于诊断哪些特定任务失败以及失败原因。
...
任务 #1: 退出码 1 (失败)
任务 #2: 退出码 0 (成功)  
任务 #3: 退出码 139 (被信号 11 终止 - SIGSEGV)
```

这种细粒度的报告能够快速识别哪些任务失败，并有助于调试多任务作业问题。

...
possible_nodes = ["db01", "db02", "db03"]
written = 0

with open("tasks_bulk.json", "w") as f:
    while current_time <= end_time and written < max_total:
        ...
        for _ in range(num_per_hour):
            ...
            nodes_num = random.randint(1, len(possible_nodes))
            nodesname = random.sample(possible_nodes, nodes_num)
            ...
            record = {
                ...
                "state": 2,
                "timelimit": 70,
                "time_submit": time_start - random.randint(10, 100),

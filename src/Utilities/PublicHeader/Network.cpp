...
class HostsMap {
 public:
  HostsMap() {
    if (!ParseHosts_())
      CRANE_ERROR("default", "Failed to parse file /etc/hosts, DNS service will be used");
  }
 ...
 private:
  bool ParseHosts_() {
    ...
    if (!hosts_ifstream.is_open()) {
      CRANE_ERROR("default", "Failed to parse hosts from file {}", kHostFilePath);
      return false;
    }
    ...
    while (getline(hosts_ifstream, line)) {
      ...
      if (!parts.empty()) {
        ...
        for (size_t i = 1; i < parts.size(); ++i) {
          ...
          case -1:
            CRANE_ERROR("default", "host file line \'{}\' parse failed", line);
            break;
          case 4:
            ipv4_t ipv4;
...
bool ResolveHostnameFromIpv4(ipv4_t addr, std::string* hostname) {
  ...
  int r = getnameinfo((struct sockaddr*)&sa, len, hbuf, sizeof(hbuf), nullptr,
                      0, NI_NAMEREQD);
  if (r != 0) {
    CRANE_TRACE("default", "Error in getnameinfo when resolving hostname for {}: {}",
...
bool ResolveIpv4FromHostname(const std::string& hostname, ipv4_t* addr) {
  ...
  if (ret != 0) {
    CRANE_WARN("default", "Error in getaddrinfo when resolving ipv4 from hostname {}: {}",
  ...
  for (struct addrinfo* tmp = res; tmp != nullptr; tmp = tmp->ai_next) {
    ...
    if (ret == 0)
      *addr =
      ...
      CRANE_ERROR("default", "error getnameinfo {}", gai_strerror(ret));
  }
...
bool ResolveIpv6FromHostname(const std::string& hostname, ipv6_t* addr) {
  ...
  int ret = getaddrinfo(hostname.c_str(), nullptr, &hints, &res);
  if (ret != 0) {
    CRANE_WARN("default", "Error in getaddrinfo when resolving ipv6 from hostname {}: {}",
               hostname, gai_strerror(ret));
    return false;
  }
...
bool StrToIpv4(const std::string& ip, ipv4_t* addr) {
  ...
  if (inet_pton(AF_INET, ip.c_str(), &ipv4_addr) != 1) {
    CRANE_ERROR("default", "Error converting ipv4 {} to uint32_t", ip);
    return false;
  }
...
bool StrToIpv6(const std::string& ip, ipv6_t* addr) {
  ...
  if (inet_pton(AF_INET6, ip.c_str(), &ipv6_addr) != 1) {
    CRANE_ERROR("default", "Error converting ipv6 {} to uint128", ip);
    return false;
  }
...
bool FindTcpInodeByPort(const std::string& tcp_path, int port, ino_t* inode) {
  ...
  if (tcp_in) {
    ...
    while (getline(tcp_in, tcp_line)) {
      ...
      CRANE_TRACE("default", "Checking port {} == {}", port_hex, tcp_line_vec[2]);
      if (port_hex == tcp_line_vec[2]) {
        *inode = std::stoul(tcp_line_vec[13]);
        CRANE_TRACE("default", "Inode num for port {} is {}", port, *inode);
        return true;
      }
    }
  } else {  // can't find file
    CRANE_ERROR("default", "Can't open file: {}", tcp_path);
  }
  return false;
}

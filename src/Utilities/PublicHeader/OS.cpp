...
bool DeleteFile(std::string const& p) {
  ...
  if (!ok) CRANE_ERROR("Default", "Failed to remove file {}: {}", p, ec.message());
...
bool CreateFolders(std::string const& p) {
  ...
  if (!ok) CRANE_ERROR("Default", "Failed to create folder {}: {}", p, ec.message());
...
bool CreateFoldersForFile(std::string const& p) {
  try {
    ...
    CRANE_ERROR("Default", "Failed to create folder for {}: {}", p, e.what());
    return false;
  }
...
absl::Time GetSystemBootTime() {
  ...
  if (sysinfo(&system_info) != 0) {
    CRANE_ERROR("Default", "Failed to get sysinfo {}.", strerror(errno));
    return {};
  }

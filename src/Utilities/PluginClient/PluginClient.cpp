...
PluginClient::~PluginClient() {
  ...
  CRANE_TRACE("Default", "PluginClient is ending. Waiting for the thread to finish.");
...
void PluginClient::AsyncSendThread_() {
  ...
  while (true) {
    ...
    if (!prev_conn_state && connected) {
      CRANE_INFO("Default", "[Plugin] Plugind is connected.");
    }
    prev_conn_state = connected;

    if (!connected) {
      CRANE_INFO("Default", "[Plugin] Plugind is not connected. Reconnecting...");
    ...
    CRANE_DEBUG("Default", "[Plugin] Dequeued {} hook events.", actual_size);
    ...
    while (!events.empty()) {
      ...
      if (!status.ok()) {
        ...
        CRANE_TRACE("Default", "[Plugin] Hook event sent: hook type: {}", int(e.type));
      }
...
grpc::Status PluginClient::SendStartHook_(grpc::ClientContext* context,
  ...
  CRANE_TRACE("Default", "[Plugin] Sending StartHook.");
...
grpc::Status PluginClient::SendEndHook_(grpc::ClientContext* context,
  ...
  CRANE_TRACE("Default", "[Plugin] Sending EndHook.");
...
grpc::Status PluginClient::SendJobMonitorHook_(grpc::ClientContext* context,
  ...
  CRANE_TRACE("Default", "[Plugin] Sending JobMonitorHook.");

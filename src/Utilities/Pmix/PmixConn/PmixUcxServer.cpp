...
PmixUcxMsgType PmixUcxServer::TagToType_(uint64_t tag) {
  return static_cast<PmixUcxMsgType>(
      (tag & kTagTypeMask) >> kTagTypeShift);
}
...
void PmixUcxServiceImpl::SendPmixRingMsg(const std::string& data) {
  ...
  if (!req.ParseFromString(data)) {
    CRANE_ERROR("SendPmixRingMsg: ParseFromString failed"); return;
  }
...
void PmixUcxServiceImpl::PmixTreeUpwardForward(const std::string& data) {
  ...
  if (!req.ParseFromString(data)) {
    CRANE_ERROR("PmixTreeUpwardForward: ParseFromString failed"); return;
  }
  std::vector<pmix_proc_t> procs;
  FillProcs(req.pmix_procs(), procs);
...
void PmixUcxServiceImpl::PmixTreeDownwardForward(const std::string& data) {
  ...
  if (!req.ParseFromString(data)) {
    CRANE_ERROR("PmixTreeDownwardForward: ParseFromString failed"); return;
  }
  std::vector<pmix_proc_t> procs;
  FillProcs(req.pmix_procs(), procs);
  auto coll = m_pmix_state_->PmixStateCollGet(CollType::FENCE_TREE, procs);
...
void PmixUcxServiceImpl::PmixDModexRequest(const std::string& data) {
  ...
  if (!req.ParseFromString(data)) {
    CRANE_ERROR("PmixDModexRequest: ParseFromString failed"); return;
  }
  pmix_proc_t proc;
  ...
  m_dmodex_mgr_->PmixProcessRequest(
      req.seq_num(), req.craned_id(), proc, req.local_namespace());
}

void PmixUcxServiceImpl::PmixDModexResponse(const std::string& data) {
  crane::grpc::pmix::PmixDModexResponseReq req;
  if (!req.ParseFromString(data)) {
    CRANE_ERROR("PmixDModexResponse: ParseFromString failed"); return;
  }
  m_dmodex_mgr_->PmixProcessResponse(
      req.seq_num(), req.craned_id(), req.data(), req.status());
}
...
bool PmixUcxServer::Init(const Config& config) {
  ...
  m_ucx_poll_->on<uvw::poll_event>(
      [this](const uvw::poll_event&, uvw::poll_handle&) {
        OnUcxReadable_();
      });
...

  {
    ...
    if (arm_st == UCS_ERR_BUSY) {
      // arm 前已有待处理事件, 先 progress 消化, 再重新 arm
...
void PmixUcxServer::RecvHandle_(void*                      ucp_req,
                                ucs_status_t               status,
  ...
  if (status == UCS_OK && info != nullptr) {
    ...
    CRANE_TRACE("UCX RecvHandle_ OK: type={}, len={}",
                static_cast<int>(type), info->length);
  ...
  while (m_req_queue_.try_dequeue(req)) {
    ...
    switch (req->type) {
      case PmixUcxMsgType::PMIX_UCX_SEND_PMIX_RING_MSG:
        m_service_impl_->SendPmixRingMsg(req->data);         break;
      case PmixUcxMsgType::PMIX_UCX_TREE_UPWARD_FORWARD:
        m_service_impl_->PmixTreeUpwardForward(req->data);   break;
      case PmixUcxMsgType::PMIX_UCX_TREE_DOWNWARD_FORWARD:
        m_service_impl_->PmixTreeDownwardForward(req->data); break;
      case PmixUcxMsgType::PMIX_UCX_DMDEX_REQUEST:
        m_service_impl_->PmixDModexRequest(req->data);       break;
      case PmixUcxMsgType::PMIX_UCX_DMDEX_RESPONSE:
        m_service_impl_->PmixDModexResponse(req->data);      break;
      default:
        CRANE_ERROR("Unknown UCX msg type: {}",
                    static_cast<int>(req->type));             break;
    }
    delete req;
  }
}

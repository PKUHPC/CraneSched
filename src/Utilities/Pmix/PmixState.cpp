...
std::shared_ptr<Coll> PmixState::PmixStateCollGet(CollType type,
                                  const std::vector<pmix_proc_t>& procs,
                                  size_t nprocs) {

  std::lock_guard<std::mutex> lock_guard(m_mutex_);
  ...
  for (const auto& coll : m_coll_list_) {
    ...
    for (size_t i = 0; i<nprocs; i++) {
      ...
      if (std::strcmp(proc.nspace, procs[i].nspace) == 0 && proc.rank == procs[i].rank)
        return coll;
    }
  }

  std::shared_ptr<Coll> coll;
  if (!coll->PmixCollInit(type, procs, nprocs))
    return nullptr;

...
bool PmixCollRing::PmixCollContribLocal(const std::string& data, pmix_modex_cbfunc_t cbfunc, void* cbdata) {
    ...
    stub->SendPmixRingMsgNoBlock(request,
      [seq = coll_ring_ctx.seq, &coll_ring_ctx, self](bool ok) {
                            ...
                            if (!ok) {
                              ...
                              PmixLibModexInvoke(NULL, PMIX_ERR_TIMEOUT, nullptr, 0, nullptr, nullptr, nullptr);
                              return;
                            }

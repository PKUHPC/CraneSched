...
static int GeometricSeries(int width, int depth) {
   ...
   * (depth+1).
   */
  return width == 1 ? depth + 1 : (1 - InitPow(width, depth + 1)) / (1 - width);
}
  ...
  *depth = *depth + 1;
  ...
  for (i = 1; i <= width; i++) {
    ...
    if (id > current && id < next) {
      return SearchTree(id, current, next_children, width, parent_id,
                        next_max_children, depth);
    }
    current = next;
  }
  *parent_id = -1;
...
void ReverseTreeInfo(int rank, int num_nodes, int width, int *parent,
                     int *num_children, int *depth, int *max_depth) {
  int max_children;
  int p, c;
  ...
  *depth = 0;
  ...
  if (rank + c >= num_nodes) c = num_nodes - rank - 1;

        ...
        cb_step_completed =
            [this,
             proxy_weak_ptr](StepInteractiveMeta::StepCompeteArgs const& args) {
              ...
              auto iter =
                  m_ctld_server_->m_cfored_running_jobs_.find(cfored_name);
...
grpc::Status CraneCtldServiceImpl::RequeueJob(
    grpc::ServerContext* context, const crane::grpc::RequeueJobRequest* request,
...
grpc::Status CraneCtldServiceImpl::SetTraceConfig(
  ...
  if (propagate && g_craned_keeper) {
    std::vector<CranedId> craned_ids;
    {
      ...
      craned_ids.reserve(craned_meta_map->size());
      for (const auto& craned_id : *craned_meta_map | std::views::keys)

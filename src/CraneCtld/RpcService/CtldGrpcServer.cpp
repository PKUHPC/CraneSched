      ...
      if (ok) {
        ...
        case StreamCforedRequest::TASK_REQUEST: {
          ...
          if (submit_result.has_value()) {
            CraneExpected<task_id_t> task_result = submit_result.value().get();
            if (task_result.has_value()) {
              result =
                  std::expected<task_id_t, std::string>{task_result.value()};
            } else {
              result = std::unexpected(CraneErrStr(task_result.error()));
            }
          } else {

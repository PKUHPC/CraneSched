...
uint32_t QosSubmitReserveCount(const JobInCtld& job) {
  if (!job.IsArrayParent()) return 1;
...
void EmitStepRpcPoint(std::string_view point, std::string_view flow_id,
                      std::string_view traceparent, job_id_t job_id,
                      step_id_t step_id, const CranedId& craned_id,
                      std::string_view operation, std::string_view outcome,
                      bool failed) {
  CRANE_FLOW_POINT(point, flow_id, traceparent,
                   CRANE_FLOW_SET_ATTR("job_id", job_id);
                   CRANE_FLOW_SET_ATTR("step_id", step_id);
                   CRANE_FLOW_SET_ATTR("node_id", std::string{craned_id});
                   CRANE_FLOW_SET_ATTR("operation", operation);
                   CRANE_FLOW_SET_ATTR("outcome", outcome);
                   if (failed) CRANE_FLOW_SET_ERROR(););
}
#endif
...
bool JobScheduler::Init() {
  ...
  auto fail_recovered_running_job = [this, &recovered_running_jobs](
    ...
    if (job_it == recovered_running_jobs.end()) return;
    ...
    if (job->IsArrayChild() && job->ArrayJobId().has_value()) {
      if (auto track = m_array_manager_->TrackRecoveredTerminalChild(job);
          !track.has_value()) {
        CRANE_ERROR(
            "Failed to track recovered terminal array child #{} for parent "
            "#{}: {}.",
            job_id, job->ArrayJobId().value(), CraneErrStr(track.error()));
      }
    }
    if (!g_embedded_db_client->UpdateRuntimeAttrOfJob(0, job->JobDbId(),
                                                      job->RuntimeAttr())) {
      CRANE_ERROR(
          "UpdateRuntimeAttrOfJob failed for recovered running job #{} "
          "when marking as FAILED: {}.",
          job_id, reason);
    }
    if (!g_db_client->InsertJob(job)) {
      CRANE_ERROR("InsertJob failed for recovered running job #{} fallback.",
                  job_id);
    }
    if (!g_embedded_db_client->PurgeEndedJobs(
            {{job->JobId(), job->JobDbId()}})) {
      CRANE_ERROR(
          "PurgeEndedJobs failed for recovered running job #{} fallback.",
          job_id);
    }
    recovered_running_jobs.erase(job_it);
  };
  if (!running_queue.empty()) {
    ...
    for (auto&& [job_db_id, job_in_embedded_db] : running_queue) {
      ...
      if (job->IsArrayChild()) {
        recovered_array_child_ids_by_parent[job->ArrayJobId().value()]
            .push_back(job_id);
      }
  ...
  // Rebuild array parent metadata after all jobs have been recovered.
  {
    LockGuard pending_guard(&m_pending_job_map_mtx_);
    m_array_manager_->RecoverAccountingForRegisteredParents();

    std::unordered_set<job_id_t> failed_recovered_array_child_ids;
    for (const auto& [parent_id, child_ids] :
         recovered_array_child_ids_by_parent) {
      if (!m_array_manager_->IsRegisteredParent(parent_id)) {
        CRANE_ERROR(
            "Recovered array child for missing parent #{}; child recovery "
            "will fall back to failed-job handling.",
            parent_id);
        failed_recovered_array_child_ids.insert(child_ids.begin(),
                                                child_ids.end());
        continue;
      }
    }
    for (job_id_t child_id : failed_recovered_array_child_ids) {
      fail_recovered_running_job(child_id, "missing array parent");
    }
  }
    ...
    if (job->IsArrayChild() && job->ArrayJobId().has_value()) {
      {
        ...
        if (auto track = m_array_manager_->TrackRecoveredTerminalChild(job);
            !track.has_value()) {
          CRANE_ERROR(
              "Failed to track invalid recovered array child #{} for parent "
              "#{}: {}.",
              job_id, job->ArrayJobId().value(), CraneErrStr(track.error()));
        }
      }
  ...
  {
    LockGuard pending_guard(&m_pending_job_map_mtx_);
    std::vector<job_id_t> failed_recovered_array_child_ids;
    for (const auto& [parent_id, child_ids] :
         recovered_array_child_ids_by_parent) {
      if (!m_array_manager_->IsRegisteredParent(parent_id)) continue;

      std::vector<JobInCtld*> children;
      children.reserve(child_ids.size());
      for (job_id_t child_id : child_ids) {
        auto child_it = recovered_running_jobs.find(child_id);
        if (child_it != recovered_running_jobs.end()) {
          children.push_back(child_it->second.get());
        }
      }
      ...
      if (auto bind = m_array_manager_->BindRecoveredRunningChildrenForParent(
        ...
        CRANE_ERROR("Failed to bind recovered array children to parent #{}",
                    parent_id);
        failed_recovered_array_child_ids.insert(
            failed_recovered_array_child_ids.end(), child_ids.begin(),
            child_ids.end());
      }
    }
    ...
    recovered_final_array_parents =
        m_array_manager_->FinalizeAllCompletedParents();
    if (!recovered_final_array_parents.empty()) {
      SpliceFinalArrayParentsFromPendingMapNoLock_(
          recovered_final_array_parents);
    }
  }
...
void JobScheduler::RequeueRecoveredJobIntoPendingQueueLock_(
  ...
  LockGuard pending_guard(&m_pending_job_map_mtx_);
  const bool is_array_parent = job->IsArrayParent();
  if (is_array_parent) {
    m_array_manager_->RegisterParent(job.get());
  }
...
void JobScheduler::ScheduleThread_() {
  ...
  while (!m_thread_stop_) {
    ...
    if (!m_pending_job_map_.empty()) {  // all_part_metas is locked here.
      ...
      if (approx_size > 0) {
        ...
        for (const auto& event : dep_events) {
          ...
          // Array parents are represented in m_pending_job_map_; if a dependent
          // is not pending anymore, there is no array-side owner to update.
        }
      }
      ...
      for (auto& it : m_pending_job_map_) {
        ...
        if (job->IsArrayParent()) {
          auto decision =
              m_array_manager_->PrepareParentForMaterialization(*job, now);
          if (!decision.can_materialize) {
      ...
      for (auto& job_in_scheduler : pending_jobs) {
        ...
        std::unique_ptr<JobInCtld> child_holder;
        JobInCtld* launch_job = job.get();
        if (job_in_scheduler->materializes_array_child) {
          child_holder = m_array_manager_->MaterializeChildForAllocation(
              job.get(), absl::Now());
          if (!child_holder) {
            if (!job_in_scheduler->actual_licenses.empty()) {
              g_license_manager->FreeLicense(job_in_scheduler->actual_licenses);
        ...
        launch_job->SetCachedPriority(job_in_scheduler->priority);
        launch_job->SetStartTime(job_in_scheduler->start_time);
        launch_job->SetEndTime(end_time);
        launch_job->SetCranedIds(std::move(job_in_scheduler->craned_ids));
        launch_job->SetStepResAvail(job_in_scheduler->allocated_res);
        launch_job->SetAllocatedRes(std::move(job_in_scheduler->allocated_res));
        launch_job->SetActualLicenses(
            std::move(job_in_scheduler->actual_licenses));
        launch_job->allocated_res_view.SetToZero();
        launch_job->allocated_res_view += launch_job->AllocatedRes();
        launch_job->nodes_alloc = launch_job->CranedIds().size();

        if (launch_job->PendingSpan().IsActive()) {
          launch_job->PendingSpan().End();
        }

        {
          CRANE_TRACE_MANUAL(lifecycle_span, "job/lifecycle");
          lifecycle_span.SetAttribute("crane.dimension", "lifecycle");
          lifecycle_span.SetAttribute("job_id", launch_job->JobId());
          lifecycle_span.SetAttribute("partition", launch_job->partition_id);
          if (!launch_job->SubmitId().empty())
            lifecycle_span.SetAttribute("crane.submit_id",
                                        launch_job->SubmitId());
          lifecycle_span.SetAttribute(
              "node_count",
              static_cast<int64_t>(launch_job->CranedIds().size()));
          launch_job->SetTraceparent(
              crane::SerializeTraceParent(lifecycle_span.GetContext()));
          launch_job->LifecycleSpan() = std::move(lifecycle_span);
        }

        launch_job->SetStatus(crane::grpc::JobStatus::Configuring);
        launch_job->allocated_craneds_regex =
            util::HostNameListToStr(launch_job->CranedIds());

#ifdef CRANE_ENABLE_EXECUTION_FLOW
        if (crane::ExecutionFlowEnabled()) {
          for (const auto& craned_id : launch_job->CranedIds()) {
            CRANE_FLOW_POINT(
                "ctld/job/allocated", launch_job->ExecutionFlowId(),
                launch_job->Traceparent(),
                CRANE_FLOW_SET_ATTR("job_id", launch_job->JobId());
                CRANE_FLOW_SET_ATTR("node_id", std::string{craned_id});
                CRANE_FLOW_SET_ATTR("operation", "allocate");
                CRANE_FLOW_SET_ATTR("outcome", "success"););
          }
        }
#endif

        for (CranedId const& craned_id : launch_job->CranedIds())
          g_meta_container->MallocResourceFromNode(
              craned_id, launch_job->JobId(), launch_job->AllocatedRes());
        if (launch_job->reservation != "") {
          g_meta_container->MallocResourceFromResv(launch_job->reservation,
                                                   launch_job->JobId(),
                                                   launch_job->AllocatedRes());
        }

        if (launch_job->ShouldLaunchOnAllNodes()) {
          for (auto const& craned_id : launch_job->CranedIds())
            launch_job->executing_craned_ids.emplace_back(craned_id);
        } else {
          launch_job->executing_craned_ids.emplace_back(
              launch_job->CranedIds().front());
        }

        if (job_in_scheduler->materializes_array_child) {
          jobs_to_run.push_back(std::move(child_holder));
        } else {
...
#ifdef CRANE_ENABLE_EXECUTION_FLOW
      else if (crane::ExecutionFlowEnabled()) {
        for (const auto& job : jobs_to_run) {
          CRANE_FLOW_POINT(
              "ctld/job/allocation_persisted", job->ExecutionFlowId(),
              job->Traceparent(), CRANE_FLOW_SET_ATTR("job_id", job->JobId());
              CRANE_FLOW_SET_ATTR("operation", "embedded-db-commit");
              CRANE_FLOW_SET_ATTR("outcome", "success"););
        }
      }
#endif
        ...
        m_rpc_worker_pool_->detach_task([&, diag_craned_id, task_enqueue_time,
          ...
          if (stub == nullptr || stub->Invalid()) {
            ...
            for (const auto& job_to_d : jobs) {
              ...
              CRANE_FLOW_POINT(
                  "ctld/job/alloc_rpc_result", job_to_d.execution_flow_id(),
                  job_to_d.traceparent(),
                  CRANE_FLOW_SET_ATTR("job_id", job_to_d.job_id());
                  CRANE_FLOW_SET_ATTR("node_id", std::string{diag_craned_id});
                  CRANE_FLOW_SET_ATTR("operation", "alloc-jobs");
                  CRANE_FLOW_SET_ATTR("outcome", "craned-down");
          ...
          for (const auto& job_to_d : jobs) {
            ...
            CRANE_FLOW_POINT(
                ...
                CRANE_FLOW_SET_ATTR("node_id", std::string{diag_craned_id});
                CRANE_FLOW_SET_ATTR("operation", "alloc-jobs");
                CRANE_FLOW_SET_ATTR("outcome", "rpc-failure");
                CRANE_FLOW_SET_ERROR(););
          }
      ...
      for (const auto& craned_id : craned_alloc_steps | std::views::keys) {
        m_rpc_worker_pool_->detach_task([&, craned_id] {
...
#ifdef CRANE_ENABLE_EXECUTION_FLOW
          auto emit_alloc_step_results = [&](std::string_view outcome,
                                             bool failed) {
            if (!crane::ExecutionFlowEnabled()) return;
            for (const auto& step_to_d : steps) {
              auto traceparent_it = alloc_traceparents.find(step_to_d.job_id());
              EmitStepRpcPoint("ctld/step/alloc_rpc_result",
                               step_to_d.execution_flow_id(),
                               traceparent_it == alloc_traceparents.end()
                                   ? std::string_view{}
                                   : std::string_view{traceparent_it->second},
                               step_to_d.job_id(), step_to_d.step_id(),
                               craned_id, "alloc-steps", outcome, failed);
            }
          };
#endif
          ...
          if (stub == nullptr || stub->Invalid()) {
            ...
            thread_pool_mtx.Unlock();
#ifdef CRANE_ENABLE_EXECUTION_FLOW
            emit_alloc_step_results("craned-down", true);
#endif
          ...
          if (err == CraneErrCode::SUCCESS) {
#ifdef CRANE_ENABLE_EXECUTION_FLOW
            emit_alloc_step_results("success", false);
#endif
            alloc_step_latch.count_down();
            return;
          }
          ...
          thread_pool_mtx.Unlock();
#ifdef CRANE_ENABLE_EXECUTION_FLOW
          emit_alloc_step_results("rpc-failure", true);
#endif
      ...
      if (!jobs_failed.empty()) {
        ...
        std::vector<ArrayManager::FinalizedArrayParent>
            failed_final_array_parents;
        for (const auto& job : jobs_failed) {
        ...
        for (auto& job : jobs_failed) {
          ...
          if (job->IsArrayChild() && job->ArrayJobId().has_value()) {
            auto parents = m_array_manager_->OnChildTerminal(
                job.get(), failed_job_raw_ptrs);
            failed_final_array_parents.insert(
                failed_final_array_parents.end(),
        ...
        SpliceFinalArrayParentsFromPendingMapNoLock_(
            failed_final_array_parents);
        sched_loop_final_array_parents.insert(
            sched_loop_final_array_parents.end(),
            std::make_move_iterator(failed_final_array_parents.begin()),
            std::make_move_iterator(failed_final_array_parents.end()));
    ...
    // Persistence & plugin RPC for finalized array parents must run after all
...
// TODO: Refactor to "collect jobs by condition, then apply changes per
// pd/rn state". The current flow looks up a single job_id and branches on
// pending/running/array-parent inline. A cleaner structure would be:
//   1) Resolve the input condition (job_id, array task filters, etc.) into
//      a set of affected jobs (parent + materialized children when needed).
//   2) Apply pd-specific changes (deadline timer, reservation checks at
//      submit-time semantics) and rn-specific changes (RPC to executing
//      craneds) separately based on each job's state.
CraneErrCode JobScheduler::ChangeJobTimeConstraint(
...

  {
    ...
    std::vector<JobInCtld*> jobs_to_update{job};

    // Time constraint changes on array parent propagate only to running
    // materialized children. Future children inherit from the parent when
    ...
    if (is_array_parent) {
      for (job_id_t child_job_id :
        ...
        if (child_it == m_running_job_map_.end()) continue;
    ...
    for (JobInCtld* target : jobs_to_update) {
      ...
      if (absl_deadline_time) {
        job_to_ctld.mutable_deadline_time()->set_seconds(
    ...
    if (!g_embedded_db_client->BeginFixedDbTransaction(&fixed_txn_id)) {
      CRANE_ERROR(
          "Failed to begin fixed-db transaction when changing time constraint "
          "of job #{}.",
          job_id);
      return CraneErrCode::ERR_UPDATE_DATABASE;
    }

    for (const auto& [target, job_to_ctld] : pending_updates) {
      if (!g_embedded_db_client->UpdateJobToCtldIfExists(
              fixed_txn_id, target->JobDbId(), job_to_ctld)) {
        CRANE_ERROR(
            "Failed to persist time constraint change for job #{} in "
            "fixed-db transaction.",
            target->JobId());
        return CraneErrCode::ERR_UPDATE_DATABASE;
      }
    }

    if (!g_embedded_db_client->CommitFixedDbTransaction(fixed_txn_id)) {
      CRANE_ERROR(
          "Failed to commit fixed-db transaction when changing time "
          "constraint of job #{}.",
          job_id);
      return CraneErrCode::ERR_UPDATE_DATABASE;
    }
    ...
    for (JobInCtld* target : jobs_to_update) {
      ...
      if (absl_deadline_time) {
        ...
        target->MutableJobToCtld()->mutable_deadline_time()->set_seconds(
            absl::ToUnixSeconds(absl_deadline_time.value()));
      }
    }
...
CraneErrCode JobScheduler::ChangeJobExtraAttrs(
  ...
  JobInCtld* job = nullptr;
  if (pd_iter != m_pending_job_map_.end()) {
    job = pd_iter->second.get();
  } else {
    auto rn_iter = m_running_job_map_.find(job_id);
    if (rn_iter != m_running_job_map_.end()) {
      job = rn_iter->second.get();
    }
  }
  ...
  if (job->IsArrayParent()) {
    ...
    // Extra attr changes on array parent propagate only to running
    // materialized children. Future children inherit from the parent when
    // they are materialized.
    for (job_id_t child_id :
         m_array_manager_->RunningChildJobIdsForParent(job_id)) {
      auto child_it = m_running_job_map_.find(child_id);
      if (child_it == m_running_job_map_.end()) {
        continue;
      }
      jobs_to_update.emplace_back(child_it->second.get());
    }
  }
  ...
  for (JobInCtld* target : jobs_to_update) {
    crane::grpc::JobToCtld job_to_ctld = target->JobToCtld();
    job_to_ctld.set_extra_attr(new_extra_attr);
    pending_updates.emplace_back(target, std::move(job_to_ctld));
  }
  ...
  if (!g_embedded_db_client->BeginFixedDbTransaction(&fixed_txn_id)) {
    CRANE_ERROR(
        "Failed to begin fixed-db transaction when changing extra attrs of job "
        "#{}.",
        job_id);
    return CraneErrCode::ERR_UPDATE_DATABASE;
  }
  ...
  for (const auto& [target, job_to_ctld] : pending_updates) {
    if (!g_embedded_db_client->UpdateJobToCtldIfExists(
            fixed_txn_id, target->JobDbId(), job_to_ctld)) {
      CRANE_ERROR(
          "Failed to persist extra attrs change for job #{} in fixed-db "
          "transaction.",
          target->JobId());
      return CraneErrCode::ERR_UPDATE_DATABASE;
    }
  ...
  if (!g_embedded_db_client->CommitFixedDbTransaction(fixed_txn_id)) {
    CRANE_ERROR(
        "Failed to commit fixed-db transaction when changing extra attrs of "
        "job "
        "#{}.",
        job_id);
    return CraneErrCode::ERR_UPDATE_DATABASE;
  ...
  for (JobInCtld* target : jobs_to_update) {
    target->extra_attr = new_extra_attr;
    target->MutableJobToCtld()->set_extra_attr(new_extra_attr);
  }
...
void JobScheduler::JobModifyLuaCheck(
  ...
  for (const auto& selector : request.job_ids()) {
    ...
    // Array parents are held in the pending map, so no ArrayManager fallback is
    // needed here.
  }
...
void JobScheduler::CollectJobIdsForModify(
  ...
  auto rejected_parent_view =
      job_steps | ranges::views::filter(is_array_parent_with_steps);
  ...
  if (g_config.JobSubmitLuaScript.empty()) {
    *job_ids = std::move(resolved_ids);
    ...
    crane::grpc::ModifyJobRequest resolved_request(request);
    resolved_request.clear_job_ids();
    for (job_id_t job_id : resolved_ids) {
      resolved_request.add_job_ids()->set_job_id(job_id);
    }
    JobModifyLuaCheck(resolved_request, response, job_ids);
  }
}
...
JobScheduler::SubmitJobToScheduler(std::unique_ptr<JobInCtld> job) {
  ...
  validate_span.AddEvent("account_validated");
...
void JobScheduler::CancelRunningJobNoLock_(
    JobInCtld* job, crane::grpc::TerminateSource terminate_source) {
  job_id_t job_id = job->JobId();
  if (job->Status() == crane::grpc::JobStatus::Configuring) {
    ...
    CRANE_INFO(
        "[Job #{}] Cancel requested during Configuring state. Deferring until "
        "all nodes finish configuring.",
        job_id);
    return;
  }
  ...
  if (!primary_step) {
    CRANE_DEBUG(
        "[Job #{}] Primary step not found when cancelling running job, maybe "
        "completing.",
        job_id);
    return;
  }
  ...
  TerminateRunningStepNoLock_(primary_step, terminate_source);
}
...
crane::grpc::CancelJobReply JobScheduler::CancelPendingOrRunningJob(
  ...
  bool has_explicit_job_filter = !request.filter_job_ids().empty();
  std::unordered_set<job_id_t> not_found_job_ids;
  ...
  auto fill_user_id = [&](auto* entry, job_id_t job_id) {
    auto it = array_selector_by_child_job_id.find(job_id);
    if (it != array_selector_by_child_job_id.end()) {
  ...
  auto fn_cancel_pending_job = [&](job_id_t job_id) {
    ...
    if (job->IsArrayParent()) {
      ...
      auto step_it = job_steps.find(job_id);
      if (step_it != job_steps.end() && !step_it->second.empty()) {
  ...
  auto fn_cancel_running_job = [&](JobInCtld* job) {
    ...
    if (!job_steps.contains(job_id) || job_steps[job_id].empty()) {
      add_cancelled(job_id);
      CancelRunningJobNoLock_(job, crane::grpc::TERMINATE_SOURCE_USER_CANCEL);
      return;
    }

    std::vector<CommonStepInCtld*> cancel_steps;
    for (step_id_t step_id : job_steps[job_id]) {
    ...
    if (cancel_steps.empty()) return;

    auto* cancelled_entry = add_cancelled(job_id);
    for (CommonStepInCtld* step : cancel_steps) {
      ...
      cancelled_entry->add_steps(step->StepId());
    }
  };
...

  {
    ...
    if (!has_explicit_job_filter) {
      ...
      pd_input_rng =
          job_steps | ranges::views::keys |
          ranges::views::transform(
              [this, &not_found_job_ids](job_id_t job_id) -> JobInCtld* {
                auto pd_it = m_pending_job_map_.find(job_id);
                if (pd_it != m_pending_job_map_.end()) {
                  not_found_job_ids.erase(job_id);
  ...
  if (has_explicit_job_filter) {
    for (job_id_t job_id : not_found_job_ids) {
      add_not_cancelled(job_id, "Job not found");
    }
  }
  return reply;
...
void JobScheduler::CleanCancelJobQueueCb_() {
  ...
  std::vector<CancelArrayParentQueueElem> array_parents_to_cancel;
  HashMap<CranedId,
  ...
  // running jobs, batch TerminateSteps RPCs by Craned.
  for (auto& elem : jobs_to_cancel) {
    std::visit(  //
        VariantVisitor{
            ...
            [&](CancelArrayParentQueueElem& array_elem) {
              array_parents_to_cancel.emplace_back(array_elem);
            },
        },
        elem);
  }

  std::vector<ArrayManager::FinalizedArrayParent> final_array_parents;
  if (!array_parents_to_cancel.empty()) {
    LockGuard pending_guard(&m_pending_job_map_mtx_);
    LockGuard running_guard(&m_running_job_map_mtx_);
    ...
    for (const auto& array_elem : array_parents_to_cancel) {
      ...
      if (!finish_result.parent_found) continue;

      if (array_elem.terminate_running_children) {
        for (job_id_t child_id : finish_result.running_child_job_ids) {
          auto rn_it = m_running_job_map_.find(child_id);
          if (rn_it == m_running_job_map_.end()) continue;
          CancelRunningJobNoLock_(rn_it->second.get(),
                                  crane::grpc::TERMINATE_SOURCE_USER_CANCEL);
        }
      }
      ...
      final_array_parents.insert(
          ...
          std::make_move_iterator(finish_result.final_parents.begin()),
          std::make_move_iterator(finish_result.final_parents.end()));
    }
  ...
  if (!pending_job_ptr_vec.empty()) {
...
#ifdef CRANE_ENABLE_EXECUTION_FLOW
      if (job->ExecutionFlowUnsupportedReason().empty()) {
        CRANE_FLOW_POINT(
            "ctld/job/unsupported", job->RequestedExecutionFlowId(),
            job->Traceparent(), CRANE_FLOW_SET_ATTR("job_id", job->JobId());
            CRANE_FLOW_SET_ATTR("operation", "cancel-pending-job");
            CRANE_FLOW_SET_ATTR("outcome", "unsupported");
            CRANE_FLOW_SET_ATTR("reason_code", "cancelled-before-allocation"););
      }
#endif
    ...
    const auto pending_final_result = ProcessFinalJobs_(pd_job_raw_ptrs);
#ifdef CRANE_ENABLE_EXECUTION_FLOW
    // Pending jobs have no step status-change callback to close their flow.
    // Emit the terminal point only after final persistence succeeds, matching
    // the running-job path below and preserving the batch contract filter in
    // JobInCtld::ExecutionFlowId().
    if (crane::ExecutionFlowEnabled() && pending_final_result.ok) {
      for (const auto* job : pd_job_raw_ptrs) {
        CRANE_FLOW_POINT(
            "ctld/job/terminal", job->ExecutionFlowId(), job->Traceparent(),
            CRANE_FLOW_SET_ATTR("job_id", job->JobId());
            CRANE_FLOW_SET_ATTR("status", static_cast<int64_t>(job->Status()));
            CRANE_FLOW_SET_ATTR("operation", "job-terminal");
            CRANE_FLOW_SET_ATTR("outcome", "persisted"););
      }
    }
#endif
  }

  if (!final_array_parents.empty()) {
...
void JobScheduler::CleanSubmitJobQueueCb_() {
  ...
  do {
    ...
    accepted_jobs.resize(accepted_actual_size);
...
#ifdef CRANE_ENABLE_EXECUTION_FLOW
      const auto unsupported_reason = job->ExecutionFlowUnsupportedReason();
      if (unsupported_reason.empty()) {
        CRANE_FLOW_POINT("ctld/job/accepted", job->RequestedExecutionFlowId(),
                         job->SubmitTraceparent(),
                         CRANE_FLOW_SET_ATTR("job_id", job->JobId());
                         CRANE_FLOW_SET_ATTR("operation", "submit");
        ...
        CRANE_FLOW_POINT(
            ...
            CRANE_FLOW_SET_ATTR("operation", "submit");
            CRANE_FLOW_SET_ATTR("outcome", "unsupported");
            CRANE_FLOW_SET_ATTR("reason_code", unsupported_reason););
      }
#endif

      m_pending_job_map_.emplace(id, std::move(accepted_jobs[pos].first));
      if (raw->IsArrayParent()) {
...
#ifdef CRANE_ENABLE_EXECUTION_FLOW
      if (crane::ExecutionFlowEnabled()) {
        auto* common_step = static_cast<CommonStepInCtld*>(step.get());
        CRANE_FLOW_POINT(
            "ctld/job/unsupported",
            common_step->job->RequestedExecutionFlowId(),
            common_step->job->Traceparent(),
            CRANE_FLOW_SET_ATTR("job_id", common_step->job_id);
            CRANE_FLOW_SET_ATTR("step_id", common_step->StepId());
            CRANE_FLOW_SET_ATTR("operation", "submit-common-step");
            CRANE_FLOW_SET_ATTR("outcome", "unsupported");
            CRANE_FLOW_SET_ATTR("reason_code", "extra-common-step"););
      }
#endif
...
void JobScheduler::CleanJobStatusChangeQueueCb_() {
...
#ifdef CRANE_ENABLE_EXECUTION_FLOW
  const bool execution_flow_enabled = crane::ExecutionFlowEnabled();
#endif
  ...
  std::vector<ArrayManager::FinalizedArrayParent> final_array_parents;
  final_array_parents.reserve(actual_size);

  {
    ...
    for (const auto& [job_id, step_id, exit_code, new_status, craned_index,
      ...
      CRANE_FLOW_POINT(
          "ctld/status/received", job->ExecutionFlowIdForStep(step_id),
          job->Traceparent(), CRANE_FLOW_SET_ATTR("job_id", job_id);
          CRANE_FLOW_SET_ATTR("step_id", step_id);
          CRANE_FLOW_SET_ATTR("node_id", std::string{craned_index});
          CRANE_FLOW_SET_ATTR("status", static_cast<int64_t>(new_status));
          CRANE_FLOW_SET_ATTR("operation", "status-change"););
      ...
      CRANE_FLOW_POINT(
          "ctld/status/applied", job->ExecutionFlowIdForStep(step_id),
          job->Traceparent(), CRANE_FLOW_SET_ATTR("job_id", job_id);
          CRANE_FLOW_SET_ATTR("step_id", step_id);
          CRANE_FLOW_SET_ATTR("node_id", std::string{craned_index});
          CRANE_FLOW_SET_ATTR("status", static_cast<int64_t>(new_status));
          CRANE_FLOW_SET_ATTR("operation", "state-machine");
          CRANE_FLOW_SET_ATTR("outcome", "applied"););
      ...
      if (job_finished_status.has_value()) {
        ...
        if (should_requeue) {
          ...
          CRANE_FLOW_POINT(
              "ctld/job/resources_released", job->ExecutionFlowId(),
              job->Traceparent(), CRANE_FLOW_SET_ATTR("job_id", job->JobId());
              CRANE_FLOW_SET_ATTR("operation", "release-resources");
              CRANE_FLOW_SET_ATTR("outcome", "completed"););
  ...
  // Copy correlation before completed jobs leave the running map. Detached RPC
  // workers must never retain pointers to JobInCtld.
  auto remember_job_correlation = [&](const JobInCtld* job) {
    context.job_traceparents[job->JobId()] = job->Traceparent();
#ifdef CRANE_ENABLE_EXECUTION_FLOW
    if (execution_flow_enabled) {
      auto flow_id = job->ExecutionFlowId();
      if (!flow_id.empty())
        context.job_execution_flow_ids[job->JobId()] = std::move(flow_id);
    }
#endif
  };
  for (auto* job : context.rn_job_raw_ptrs) remember_job_correlation(job);
  for (auto* job : context.job_raw_ptrs) remember_job_correlation(job);
  {
    LockGuard running_guard(&m_running_job_map_mtx_);
    auto remember_running_job = [&](job_id_t job_id) {
      if (context.job_traceparents.contains(job_id)
#ifdef CRANE_ENABLE_EXECUTION_FLOW
          && (!execution_flow_enabled ||
              context.job_execution_flow_ids.contains(job_id))
#endif
      )
        return;
      auto it = m_running_job_map_.find(job_id);
      if (it != m_running_job_map_.end())
        remember_job_correlation(it->second.get());
    };
    for (const auto& craned_entry : context.craned_step_exec_map) {
      for (const auto& job_entry : craned_entry.second)
        remember_running_job(job_entry.first);
    }
#ifdef CRANE_ENABLE_EXECUTION_FLOW
    if (execution_flow_enabled) {
      ...
      for (const auto& craned_entry : context.craned_jobs_to_free) {
        for (const auto job_id : craned_entry.second)
          remember_running_job(job_id);
      }
    }
#endif
  }
  ...
  for (auto& [craned_id, steps] : context.craned_step_alloc_map) {
    ...
    if (execution_flow_enabled) {
      for (const auto& step : steps) {
        if (auto it = context.job_traceparents.find(step.job_id());
            it != context.job_traceparents.end())
          traceparents.try_emplace(step.job_id(), it->second);
      }
    }
    m_rpc_worker_pool_->detach_task([this, craned_id, steps = std::move(steps),
                                     execution_flow_enabled,
                                     traceparents = std::move(traceparents)] {
#else
    m_rpc_worker_pool_->detach_task(
      ...
      auto stub = g_craned_keeper->GetCranedStub(craned_id);
      bool success = false;
      bool craned_down = !stub || stub->Invalid();
      if (stub && !stub->Invalid()) {
        auto err = stub->AllocSteps(steps);
        success = err == CraneErrCode::SUCCESS;
        if (!success) {
          CRANE_ERROR(
...
#ifdef CRANE_ENABLE_EXECUTION_FLOW
      if (execution_flow_enabled) {
        const std::string_view outcome = success       ? "success"
                                         : craned_down ? "craned-down"
                                                       : "rpc-failure";
        for (const auto& step : steps) {
          auto traceparent_it = traceparents.find(step.job_id());
          std::string_view traceparent =
              traceparent_it == traceparents.end()
                  ? std::string_view{}
                  : std::string_view{traceparent_it->second};
          EmitStepRpcPoint("ctld/step/alloc_rpc_result",
                           step.execution_flow_id(), traceparent, step.job_id(),
                           step.step_id(), craned_id, "alloc-steps", outcome,
                           !success);
        }
      }
#endif
      // Preserve the pre-instrumentation state machine: a missing Craned
      // forces the step to Failed, while an AllocSteps RPC error remains
      // log-only here.
      if (craned_down) {
        auto now = google::protobuf::util::TimeUtil::GetCurrentTime();
        for (const auto& step : steps) {
  ...
  for (auto& [craned_id, steps] : context.craned_step_free_map) {
#ifdef CRANE_ENABLE_EXECUTION_FLOW
    std::unordered_map<job_id_t, std::string> flow_ids;
    std::unordered_map<job_id_t, std::string> traceparents;
    if (execution_flow_enabled) {
      for (const auto& job_steps : steps) {
        const auto job_id = job_steps.first;
        if (auto it = context.job_execution_flow_ids.find(job_id);
            it != context.job_execution_flow_ids.end())
          flow_ids.emplace(job_id, it->second);
        if (auto it = context.job_traceparents.find(job_id);
            it != context.job_traceparents.end())
          traceparents.emplace(job_id, it->second);
      }
    }
#endif
#ifdef CRANE_ENABLE_EXECUTION_FLOW
    ...
    m_rpc_worker_pool_->detach_task([craned_id, steps = std::move(steps)] {
      ...
      auto stub = g_craned_keeper->GetCranedStub(craned_id);
#ifdef CRANE_ENABLE_EXECUTION_FLOW
      std::string_view outcome = "craned-down";
#endif
      if (stub && !stub->Invalid()) {
      ...
      if (execution_flow_enabled) {
        for (const auto& [job_id, step_ids] : steps) {
          auto flow_it = flow_ids.find(job_id);
          if (flow_it == flow_ids.end()) continue;
          auto traceparent_it = traceparents.find(job_id);
          std::string_view traceparent = traceparent_it == traceparents.end()
                                             ? std::string_view{}
                                             : traceparent_it->second;
          for (const auto step_id : step_ids) {
            if (step_id != kDaemonStepId && step_id != kPrimaryStepId) continue;
            CRANE_FLOW_POINT(
                "ctld/step/free_rpc_result", flow_it->second, traceparent,
                CRANE_FLOW_SET_ATTR("job_id", job_id);
                CRANE_FLOW_SET_ATTR("step_id", step_id);
                CRANE_FLOW_SET_ATTR("node_id", std::string{craned_id});
                CRANE_FLOW_SET_ATTR("operation", "free-steps");
                CRANE_FLOW_SET_ATTR("outcome", outcome);
                if (outcome != "success") CRANE_FLOW_SET_ERROR(););
          }
        }
      }
#endif
  ...
  for (auto& [craned_id, steps] : context.craned_step_exec_map) {
    // Collect traceparents before moving steps into lambda
    ...
    m_rpc_worker_pool_->detach_task([this, craned_id, steps = std::move(steps),
      ...
      if (execution_flow_enabled) {
        for (const auto& [job_id, step_ids] : steps) {
          ...
          auto traceparent_it = traceparents.find(job_id);
          for (const auto step_id : step_ids) {
            EmitStepRpcPoint("ctld/step/execute_requested",
                             flow_it == flow_ids.end()
                                 ? std::string_view{}
                                 : std::string_view{flow_it->second},
                             traceparent_it == traceparents.end()
                                 ? std::string_view{}
                                 : std::string_view{traceparent_it->second},
                             job_id, step_id, craned_id, "execute-steps",
                             "requested", false);
          }
        }
      }
#endif
      if (stub && !stub->Invalid()) {
        auto first_job_id = steps.begin()->first;
        ...
        if (!failed_steps.has_value()) {
          if (execution_flow_enabled) {
            for (const auto& [job_id, step_ids] : steps) {
              for (const auto step_id : step_ids) {
                auto flow_it = flow_ids.find(job_id);
                auto traceparent_it = traceparents.find(job_id);
                EmitStepRpcPoint("ctld/step/execute_rpc_result",
                                 flow_it == flow_ids.end()
                                     ? std::string_view{}
                                     : std::string_view{flow_it->second},
                                 traceparent_it == traceparents.end()
          ...
          const auto& rejected_steps = failed_steps.value();
          if (execution_flow_enabled) {
            for (const auto& [job_id, step_ids] : steps) {
              auto rejected_it = rejected_steps.find(job_id);
              for (const auto step_id : step_ids) {
                const bool rejected = rejected_it != rejected_steps.end() &&
                                      rejected_it->second.contains(step_id);
                auto flow_it = flow_ids.find(job_id);
                auto traceparent_it = traceparents.find(job_id);
                EmitStepRpcPoint("ctld/step/execute_rpc_result",
                                 flow_it == flow_ids.end()
        ...
        if (failed_steps.has_value() && !failed_steps.value().empty()) {
          ...
          CRANE_ERROR("Failed to ExecuteStep for [{}] steps on Node {}",
                      util::JobStepsToString(rejected_steps), craned_id);
          for (const auto& [job_id, step_ids] : rejected_steps) {
            for (const auto step_id : step_ids) {
              StepStatusChangeWithReasonAsync(
...
#ifdef CRANE_ENABLE_EXECUTION_FLOW
      if (execution_flow_enabled) {
        const std::string_view outcome = success ? "success" : "failure";
        for (const auto job_id : jobs) {
          auto flow_it = flow_ids.find(job_id);
          if (flow_it == flow_ids.end()) continue;
          auto traceparent_it = traceparents.find(job_id);
          std::string_view traceparent = traceparent_it == traceparents.end()
                                             ...
                                             : traceparent_it->second;
          CRANE_FLOW_POINT(
              "ctld/job/free_result", flow_it->second, traceparent,
              CRANE_FLOW_SET_ATTR("job_id", job_id);
              CRANE_FLOW_SET_ATTR("node_id", std::string{craned_id});
              CRANE_FLOW_SET_ATTR("operation", "free-jobs");
              CRANE_FLOW_SET_ATTR("outcome", outcome);
              if (outcome != "success") CRANE_FLOW_SET_ERROR(););
        }
      }
  ...
  rpc_fanout_span.End();
  auto rpc_fanout_enqueue_ms =
      std::chrono::duration_cast<std::chrono::milliseconds>(
          std::chrono::steady_clock::now() - rpc_fanout_begin)
          .count();
  ...
  db_commit_span.SetAttribute("state_machine_ms", state_machine_ms);
  db_commit_span.SetAttribute("rpc_fanout_enqueue_ms", rpc_fanout_enqueue_ms);
  ...
  txn_id_t txn_id;
  const size_t db_chunk_size =
      std::max<uint32_t>(1, g_config.CtldConf.StatusChangeDbCommitChunkSize);

  auto step_txn_begin = std::chrono::steady_clock::now();
  std::vector<StepInCtld*> rn_steps;
  rn_steps.reserve(context.rn_step_raw_ptrs.size());
  for (auto* step : context.rn_step_raw_ptrs) {
    if (!context.step_raw_ptrs.contains(step)) rn_steps.push_back(step);
  }
  db_commit_span.SetAttribute("step_runtime_update_count",
                              static_cast<int64_t>(rn_steps.size()));
  for (size_t begin = 0; begin < rn_steps.size(); begin += db_chunk_size) {
    size_t end = std::min(begin + db_chunk_size, rn_steps.size());
    {
    ...
    {
      CRANE_TRACE_CHILD_NAMED(updates_span, db_commit_span,
                              "status_change/db_commit_step_updates");
      updates_span.SetAttribute("chunk_size",
                                static_cast<int64_t>(end - begin));
      for (size_t i = begin; i < end; ++i) {
        auto* step = rn_steps[i];
        if (!g_embedded_db_client->UpdateRuntimeAttrOfStep(
                txn_id, step->StepDbId(), step->RuntimeAttr())) {
          CRANE_ERROR("[Job #{}.{}] Failed to call UpdateRuntimeAttrOfStep()",
                      step->job_id, step->StepId());
          updates_span.SetStatus(crane::StatusCode::kError,
                                 "step_update_failed");
        }
      }
    }
    {
      ...
      if (!ok) {
        CRANE_ERROR(
            ...
            "status change.");
        commit_span.SetStatus(crane::StatusCode::kError,
                              "step_txn_commit_failed");
        db_commit_span.SetStatus(crane::StatusCode::kError,
                                 "step_txn_commit_failed");
      }
    }
  }
  auto step_txn_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
                         std::chrono::steady_clock::now() - step_txn_begin)
                         .count();
  CRANE_TRACE("Persist step status changes to embedded db cost {} ms",
              step_txn_ms);
  db_commit_span.SetAttribute("step_runtime_update_ms", step_txn_ms);
  auto now = std::chrono::steady_clock::now();

  FinalTransferResult final_step_result;
  {
    CRANE_TRACE_CHILD_NAMED(final_steps_span, db_commit_span,
                            "status_change/process_final_steps");
    final_steps_span.SetAttribute(
        "step_count", static_cast<int64_t>(context.step_raw_ptrs.size()));
    final_step_result = ProcessFinalSteps_(context.step_raw_ptrs);
    final_steps_span.SetAttribute("ok", final_step_result.ok);
    final_steps_span.SetAttribute("persist_ms", final_step_result.persist_ms);
    final_steps_span.SetAttribute("mongo_insert_ms",
                                  final_step_result.mongo_insert_ms);
    final_steps_span.SetAttribute("purge_ms", final_step_result.purge_ms);
    if (!final_step_result.ok)
      final_steps_span.SetStatus(crane::StatusCode::kError,
  ...
  auto job_txn_begin = std::chrono::steady_clock::now();
  std::vector<JobInCtld*> rn_jobs;
  rn_jobs.reserve(context.rn_job_raw_ptrs.size());
  for (auto* job : context.rn_job_raw_ptrs) {
    if (!context.job_raw_ptrs.contains(job)) rn_jobs.push_back(job);
  }
  db_commit_span.SetAttribute("job_runtime_update_count",
                              static_cast<int64_t>(rn_jobs.size()));
  for (size_t begin = 0; begin < rn_jobs.size(); begin += db_chunk_size) {
    size_t end = std::min(begin + db_chunk_size, rn_jobs.size());
    {
      CRANE_TRACE_CHILD_NAMED(begin_span, db_commit_span,
      ...
      if (!ok) {
        CRANE_ERROR(
            "JobScheduler failed to start job transaction when clean step "
            "status change.");
        begin_span.SetStatus(crane::StatusCode::kError, "job_txn_begin_failed");
        db_commit_span.SetStatus(crane::StatusCode::kError,
                                 "job_txn_begin_failed");
        continue;
      }
    }
    {
      CRANE_TRACE_CHILD_NAMED(updates_span, db_commit_span,
                              "status_change/db_commit_job_updates");
      updates_span.SetAttribute("chunk_size",
                                static_cast<int64_t>(end - begin));
      for (size_t i = begin; i < end; ++i) {
        auto* job = rn_jobs[i];
  ...
  db_commit_span.SetAttribute("job_runtime_update_ms", job_txn_ms);
  ...
  FinalTransferResult final_job_result;
  {
    CRANE_TRACE_CHILD_NAMED(mongo_insert_span, db_commit_span,
                            "status_change/mongo_insert_jobs");
    mongo_insert_span.SetAttribute(
        "job_count", static_cast<int64_t>(context.job_raw_ptrs.size()));
    final_job_result = ProcessFinalJobs_(context.job_raw_ptrs);
#ifdef CRANE_ENABLE_EXECUTION_FLOW
    if (execution_flow_enabled && final_job_result.ok) {
      for (const auto* job : context.job_raw_ptrs) {
        CRANE_FLOW_POINT(
            "ctld/job/terminal", job->ExecutionFlowId(), job->Traceparent(),
            CRANE_FLOW_SET_ATTR("job_id", job->JobId());
            CRANE_FLOW_SET_ATTR("status", static_cast<int64_t>(job->Status()));
...
#endif
    mongo_insert_span.SetAttribute("ok", final_job_result.ok);
    mongo_insert_span.SetAttribute("persist_ms", final_job_result.persist_ms);
    mongo_insert_span.SetAttribute("mongo_insert_ms",
                                   final_job_result.mongo_insert_ms);
    mongo_insert_span.SetAttribute("mongo_bulk_upsert_ms",
                                   final_job_result.mongo_insert_ms);
    mongo_insert_span.SetAttribute("purge_ms", final_job_result.purge_ms);
    mongo_insert_span.SetAttribute("plugin_hook_enqueue_ms",
  ...
  auto total_elapsed_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
                              end_time - begin_time)
                              .count();
  db_commit_span.SetAttribute("final_step_persist_ms",
                              final_step_result.persist_ms);
  db_commit_span.SetAttribute("final_step_mongo_insert_ms",
                              final_step_result.mongo_insert_ms);
  db_commit_span.SetAttribute("final_step_purge_ms",
                              final_step_result.purge_ms);
  db_commit_span.SetAttribute("final_job_persist_ms",
  ...
  db_commit_span.SetAttribute("mongo_insert_ms",
                              final_job_result.mongo_insert_ms);
  db_commit_span.SetAttribute("mongo_bulk_upsert_ms",
                              final_job_result.mongo_insert_ms);
  db_commit_span.SetAttribute("aggregation_mode", g_config.JobAggregationMode);
  db_commit_span.SetAttribute("aggregation_pending_jobs",
                              g_db_client->PendingJobAggregationCount());
  ...
  if (total_elapsed_ms > 1000 || step_txn_ms > 1000 || job_txn_ms > 1000 ||
    ...
    CRANE_WARN(
        "StatusChangeDbCommitDiag batch_size={} step_update_count={} "
        "job_update_count={} finished_job_count={} step_txn_ms={} "
        "job_txn_ms={} mongo_insert_ms={} total_elapsed_ms={} "
        "db_chunk_size={} max_drain_per_tick={} state_machine_ms={} "
        "rpc_fanout_enqueue_ms={} final_step_persist_ms={} "
        "final_step_mongo_insert_ms={} final_step_purge_ms={} "
        ...
        final_job_result.purge_ms, final_job_result.plugin_hook_enqueue_ms,
        array_parent_ms);
  } else {
    CRANE_DEBUG(
        "StatusChangeDbCommitDiag batch_size={} step_update_count={} "
...
void JobScheduler::QueryJobsInRam(
  ...
  bool no_ids_constraint = request->filter_job_ids().empty();
  ...
  auto get_job_ptr_by_id = [&](job_id_t job_id) -> JobInCtld* {
    ...
    if (pending_it != m_pending_job_map_.end()) {
      return pending_it->second.get();
    }
    auto running_it = m_running_job_map_.find(job_id);
    if (running_it != m_running_job_map_.end()) {
      return running_it->second.get();
    }
    return nullptr;
  };
  ...
  auto append_step_fn = [&](auto* step_info_list, StepInCtld* step) {
    if (!step) return;
    auto req_step_it = req_steps.find(step->job_id);
    if (no_ids_constraint || req_step_it == req_steps.end() ||
        req_step_it->second.empty() ||
...
JobScheduler::FinalTransferResult JobScheduler::ProcessFinalSteps_(
    std::unordered_set<StepInCtld*> const& steps) {
  return PersistAndTransferStepsToMongodb_(steps);
...
JobScheduler::PersistAndTransferStepsToMongodb_(
  ...
  const size_t db_chunk_size =
      std::max<uint32_t>(1, g_config.CtldConf.StatusChangeDbCommitChunkSize);
  std::vector<StepInCtld*> step_vec(steps.begin(), steps.end());
  auto persist_begin = std::chrono::steady_clock::now();
  for (size_t begin = 0; begin < step_vec.size(); begin += db_chunk_size) {
    size_t end = std::min(begin + db_chunk_size, step_vec.size());
    txn_id_t txn_id;
    if (!g_embedded_db_client->BeginStepVarDbTransaction(&txn_id)) {
      result.ok = false;
      result.failed_stage = "step_final_txn_begin_failed";
      CRANE_ERROR("Failed to begin final step runtime transaction.");
      break;
    }
    bool chunk_ok = true;
    for (size_t i = begin; i < end; ++i) {
      StepInCtld* step = step_vec[i];
      if (!g_embedded_db_client->UpdateRuntimeAttrOfStep(
              txn_id, step->StepDbId(), step->RuntimeAttr())) {
        chunk_ok = false;
    ...
    auto commit_ok = g_embedded_db_client->CommitStepVarDbTransaction(txn_id);
    if (!chunk_ok) {
      result.ok = false;
      result.failed_stage = "step_final_runtime_update_failed";
      break;
    }
    if (!commit_ok) {
      result.ok = false;
      result.failed_stage = "step_final_txn_commit_failed";
      CRANE_ERROR("Failed to commit final step runtime transaction.");
      break;
    }
  }
  result.persist_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
                          std::chrono::steady_clock::now() - persist_begin)
                          .count();
  if (!result.ok) return result;

  auto mongo_begin = std::chrono::steady_clock::now();
  if (!g_db_client->InsertSteps(steps)) {
    result.ok = false;
    result.failed_stage = "step_final_mongo_insert_failed";
  ...
  auto purge_begin = std::chrono::steady_clock::now();
  if (!g_embedded_db_client->PurgeEndedSteps(db_ids)) {
    result.ok = false;
    result.failed_stage = "step_final_embedded_purge_failed";
    CRANE_ERROR(
        "Failed to call g_embedded_db_client->PurgeEndedSteps() "
        "for final jobs");
  }
  result.purge_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
                        std::chrono::steady_clock::now() - purge_begin)
                        .count();
  return result;
}

JobScheduler::FinalTransferResult JobScheduler::ProcessFinalJobs_(
    const std::unordered_set<JobInCtld*>& jobs) {
  FinalTransferResult result = PersistAndTransferJobsToMongodb_(jobs);
  if (result.ok || result.mongo_inserted)
    result.plugin_hook_enqueue_ms = CallPluginHookForFinalJobs_(jobs);
  return result;
}

void JobScheduler::SpliceFinalArrayParentsFromPendingMapNoLock_(
    std::vector<ArrayManager::FinalizedArrayParent>& parents) {
  m_pending_job_map_mtx_.AssertHeld();
  for (auto& bundle : parents) {
    auto it = m_pending_job_map_.find(bundle.array_job_id);
    if (it == m_pending_job_map_.end()) {
      CRANE_ERROR(
          "Finalized array parent #{} is missing from pending map during "
          "splice.",
          bundle.array_job_id);
      continue;
    }
...
int64_t JobScheduler::CallPluginHookForFinalJobs_(
    std::unordered_set<JobInCtld*> const& jobs) {
  auto begin = std::chrono::steady_clock::now();
  if (!g_config.Plugin.Enabled || jobs.empty()) return 0;

  std::vector<crane::grpc::JobInfo> jobs_post_comp;
  jobs_post_comp.reserve(jobs.size());
...
JobScheduler::PersistAndTransferJobsToMongodb_(
  ...
  const size_t db_chunk_size =
      std::max<uint32_t>(1, g_config.CtldConf.StatusChangeDbCommitChunkSize);
  std::vector<JobInCtld*> job_vec(jobs.begin(), jobs.end());
  auto persist_begin = std::chrono::steady_clock::now();
  for (size_t begin = 0; begin < job_vec.size(); begin += db_chunk_size) {
    size_t end = std::min(begin + db_chunk_size, job_vec.size());
    txn_id_t txn_id;
    if (!g_embedded_db_client->BeginVariableDbTransaction(&txn_id)) {
      result.ok = false;
      result.failed_stage = "job_final_txn_begin_failed";
      CRANE_ERROR("Failed to begin final job runtime transaction.");
      break;
    }
    bool chunk_ok = true;
    for (size_t i = begin; i < end; ++i) {
    ...
    auto commit_ok = g_embedded_db_client->CommitVariableDbTransaction(txn_id);
    if (!chunk_ok) {
      result.ok = false;
      result.failed_stage = "job_final_runtime_update_failed";
      break;
    }
    if (!commit_ok) {
      result.ok = false;
      result.failed_stage = "job_final_txn_commit_failed";
      CRANE_ERROR("Failed to commit final job runtime transaction.");
      break;
    }
  }
  result.persist_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
  ...
  if (crane::ExecutionFlowEnabled()) {
    for (const auto* job : jobs) {
      CRANE_FLOW_POINT("ctld/job/embedded_persisted", job->ExecutionFlowId(),
                       job->Traceparent(),
                       CRANE_FLOW_SET_ATTR("job_id", job->JobId());
                       CRANE_FLOW_SET_ATTR("operation", "embedded-db-finalize");
                       CRANE_FLOW_SET_ATTR("outcome", "success"););
    }
  }
#endif

  auto mongo_insert_begin = std::chrono::steady_clock::now();
  if (!g_db_client->InsertJobs(jobs)) {
    result.ok = false;
    result.failed_stage = "job_final_mongo_insert_failed";
  ...
  if (crane::ExecutionFlowEnabled()) {
    for (const auto* job : jobs) {
      CRANE_FLOW_POINT("ctld/job/mongo_persisted", job->ExecutionFlowId(),
                       job->Traceparent(),
                       CRANE_FLOW_SET_ATTR("job_id", job->JobId());
                       CRANE_FLOW_SET_ATTR("operation", "mongodb-finalize");
                       CRANE_FLOW_SET_ATTR("outcome", "success"););
    }
  }
#endif
  ...
  if (!g_embedded_db_client->PurgeEndedJobs(db_ids)) {
    result.ok = false;
    result.failed_stage = "job_final_embedded_purge_failed";
    CRANE_ERROR(

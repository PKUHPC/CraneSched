...
DaemonStepInCtld::StepStatusChange(crane::grpc::TaskStatus new_status,
  ...
  // If the reason indicates this is a status recovery, update to Craned's state
  if (reason.find("Status recovered from Craned") != std::string::npos) {
    CRANE_INFO("[Step #{}.{}] Recovering status from Craned: {} -> {}",
               job_id, this->StepId(), this->Status(), new_status);
    ...
    if (new_status == crane::grpc::TaskStatus::Completing) {
      ...
      CRANE_INFO("[Step #{}.{}] Status recovered to Completing",
                 job_id, this->StepId());
      ...
      CRANE_INFO("[Step #{}.{}] Status recovered to Running",
                 job_id, this->StepId());
      return std::nullopt;  // Job not finished yet
    }
...
void CommonStepInCtld::StepStatusChange(crane::grpc::TaskStatus new_status,
  ...
  if (reason.find("Status recovered from Craned") != std::string::npos) {
    CRANE_INFO("[Step #{}.{}] Recovering status from Craned: {} -> {}",
               job_id, step_id, this->Status(), new_status);
    ...
    // For terminal states (Completed/Failed/Cancelled), continue with normal processing
  }

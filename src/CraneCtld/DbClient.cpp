...
bool MongodbClient::CheckDefaultRootAccountUserAndInit_() {
  ...
  if (!SelectQos("name", kUnlimitedQosName, &qos)) {
    CRANECTLD_TRACE("Default Qos {} not found, crane will create it",
                kUnlimitedQosName);
    ...
    if (!InsertQos(qos)) {
      CRANECTLD_ERROR("Failed to insert default qos {}!", kUnlimitedQosName);
      return false;
    }
  }
  ...
  if (!SelectAccount("name", "ROOT", &root_account)) {
    CRANECTLD_TRACE("Default account ROOT not found. insert ROOT account into DB.");
    ...
    if (!InsertAccount(root_account)) {
      CRANECTLD_ERROR("Failed to insert default ROOT account!");
      return false;
    }
  }
  ...
  if (!SelectUser("uid", 0, &root_user)) {
    CRANECTLD_TRACE("Default user ROOT not found. Insert it into DB.");
    ...
    if (!InsertUser(root_user)) {
      CRANECTLD_ERROR("Failed to insert default user ROOT!");
      return false;
    }
  }
...
bool MongodbClient::CommitTransaction(
  ...
  try {
    ...
    CRANECTLD_ERROR("Database transaction failed: {}", e.what());
    return false;
  }
  return true;
}
...
MongodbClient::MongodbClient() {
  ...
  CRANECTLD_TRACE("Mongodb connect uri: {}", m_connect_uri_);

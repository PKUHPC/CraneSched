...
CraneExpected<void> TaskScheduler::AcquireTaskAttributes(TaskInCtld* task) {
  ...
  if (g_config.WckeyValid) {
    if (task->MutableTaskToCtld()->has_wckey() &&
        !task->MutableTaskToCtld()->wckey().wckey_name().empty()) {
      std::string wckey = task->MutableTaskToCtld()->wckey().wckey_name();
      ...
      task->MutableTaskToCtld()->mutable_wckey()->set_using_default_wckey(true);

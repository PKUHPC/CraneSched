...
TaskScheduler::TaskScheduler() {
  if (g_config.PriorityConfig.Type == Config::Priority::Basic) {
    TASKSCHEDULER_INFO("basic priority sorter is selected.");
    m_priority_sorter_ = std::make_unique<BasicPriority>();
  } else if (g_config.PriorityConfig.Type == Config::Priority::MultiFactor) {
    TASKSCHEDULER_INFO("multifactorial priority sorter is selected.");
...
bool TaskScheduler::Init() {
  ...
  if (!ok) {
    TASKSCHEDULER_ERROR("Failed to retrieve embedded DB snapshot!");
    return false;
  }
  ...
  if (!running_queue.empty()) {
    TASKSCHEDULER_INFO("{} running task(s) recovered.", running_queue.size());
    ...
    for (auto&& [task_db_id, task_in_embedded_db] : running_queue) {
      ...
      TASKSCHEDULER_TRACE("Restore task #{} from embedded running queue.",
                  task->TaskId());
    ...
    for (const auto& [craned_id, cgroups] : craned_cgroups_map) {
      g_thread_pool->detach_task([&bl, &craned_id, &cgroups]() {
        ...
        if (stub && !stub->Invalid()) {
          ...
          if (err != CraneErr::kOk) {
            TASKSCHEDULER_ERROR("Failed to Release cgroup RPC for {} tasks on Node {}",
  ...
  if (!pending_queue.empty()) {
    TASKSCHEDULER_INFO("{} pending task(s) recovered.", pending_queue.size());
    ...
    for (auto&& [task_db_id, task_in_embedded_db] : pending_queue) {
      ...
      TASKSCHEDULER_TRACE("Restore task #{} from embedded pending queue.",
                  task->TaskId());

      bool mark_task_as_failed = false;

      if (task->type != crane::grpc::Batch) {
        TASKSCHEDULER_INFO("Mark interactive task #{} as FAILED", task_id);
        mark_task_as_failed = true;
      }

      if (!mark_task_as_failed &&
          AcquireTaskAttributes(task.get()) != CraneErr::kOk) {
        TASKSCHEDULER_ERROR("AcquireTaskAttributes failed for task #{}", task_id);
        mark_task_as_failed = true;
      }

      if (!mark_task_as_failed &&
          CheckTaskValidity(task.get()) != CraneErr::kOk) {
        TASKSCHEDULER_ERROR("CheckTaskValidity failed for task #{}", task_id);
        mark_task_as_failed = true;
      }
  ...
  if (!snapshot.final_queue.empty()) {
    TASKSCHEDULER_INFO("{} final task(s) might not have been put to mongodb.",
    ...
    ok = g_embedded_db_client->PurgeEndedTasks(db_ids);
    if (!ok) {
      TASKSCHEDULER_ERROR("Failed to call g_embedded_db_client->PurgeEndedTasks()");
    }
  }
...
void TaskScheduler::ReleaseTaskThread_(
  ...
  if (idle_handle->start() != 0) {
    TASKSCHEDULER_ERROR("Failed to start the idle event in cancel loop.");
  }
...
void TaskScheduler::SubmitTaskThread_(
  ...
  if (idle_handle->start() != 0) {
    TASKSCHEDULER_ERROR("Failed to start the idle event in submit loop.");
  }
    ...
    if (!m_pending_task_map_.empty()) {  // all_part_metas is locked here.
      ...
      for (auto&& iter : craned_cgroup_map) {
        ...
        g_thread_pool->detach_task([&]() {
          auto stub = g_craned_keeper->GetCranedStub(craned_id);
          TASKSCHEDULER_TRACE("Send CreateCgroupForTasks for {} tasks to {}",
          ...
          TASKSCHEDULER_ERROR("Craned #{} failed when CreateCgroupForTasks.",
                      craned_id);
      ...
      if (!ok) {
        TASKSCHEDULER_ERROR("Embedded database failed to commit manual transaction.");
      }
      ...
      for (auto const& [craned_id, tasks] : craned_exec_requests_map) {
        auto stub = g_craned_keeper->GetCranedStub(craned_id);
        TASKSCHEDULER_TRACE("Send ExecuteTasks for {} tasks to {}", tasks.tasks_size(),
                    craned_id);
        if (stub == nullptr || stub->Invalid()) {
      ...
      for (auto& [craned_id, task_id] : failed_to_exec_task_id_set) {
        TASKSCHEDULER_ERROR("Task #{} on {} failed to execute.", task_id, craned_id);
...
CraneErr TaskScheduler::ChangeTaskTimeLimit(task_id_t task_id, int64_t secs) {
...

  {
    ...
    if (!found) {
      TASKSCHEDULER_DEBUG("Task #{} not in Pd/Rn queue for time limit change!",
                  task_id);
  ...
  for (const CranedId& craned_id : craned_ids) {
    ...
    if (stub && !stub->Invalid()) {
      ...
      if (err != CraneErr::kOk) {
        TASKSCHEDULER_ERROR("Failed to change time limit of task #{} on Node {}",
                    task_id, craned_id);
        return err;
      }
    }
...
CraneErr TaskScheduler::ChangeTaskPriority(task_id_t task_id, double priority) {
  ...
  if (pd_iter == m_pending_task_map_.end()) {
    m_pending_task_map_mtx_.Unlock();
    TASKSCHEDULER_TRACE("Task #{} not in Pd queue for priority change", task_id);
    return CraneErr::kNonExistent;
  }
...
CraneErr TaskScheduler::SetHoldForTaskInRamAndDb_(task_id_t task_id,
  ...
  if (pd_iter == m_pending_task_map_.end()) {
    m_pending_task_map_mtx_.Unlock();
    TASKSCHEDULER_TRACE("Task #{} not in Pd queue for hold/release", task_id);
    return CraneErr::kNonExistent;
  }
  ...
  if (!g_embedded_db_client->UpdateRuntimeAttrOfTaskIfExists(0, db_id,
                                                             runtime_attr))
    TASKSCHEDULER_ERROR("Failed to update runtime attr of task #{} to DB", task_id);
...
crane::grpc::CancelTaskReply TaskScheduler::CancelPendingOrRunningTask(
  ...
  auto fn_cancel_pending_task = [&](task_id_t task_id) {
    TASKSCHEDULER_TRACE("Cancelling pending task #{}", task_id);
  ...
  auto fn_cancel_running_task = [&](auto& it) {
    ...
    TASKSCHEDULER_TRACE("Cancelling running task #{}", task_id);
...
void TaskScheduler::CleanTaskTimerQueueCb_(
  ...
  for (auto& [req, promise] : timer_to_create) {
    ...
    if (secs == 0) {  // Remove timer
      TASKSCHEDULER_TRACE("Remove hold constraint timer for task #{}.", task_id);
      err = SetHoldForTaskInRamAndDb_(task_id, false);
    } else if (secs == std::numeric_limits<int64_t>::max()) {
      TASKSCHEDULER_TRACE("Add a hold constraint for task #{} without timer.", task_id);
      err = SetHoldForTaskInRamAndDb_(task_id, true);
    } else {  // Set timer
      TASKSCHEDULER_TRACE("Add a hold constraint for task #{} with {}s timer.", task_id,
                  secs);
      ...
      auto on_timer_cb = [this, task_id](const uvw::timer_event&,
        ...
        if (err != CraneErr::kOk)
          TASKSCHEDULER_ERROR("Failed to release task #{} after hold.", task_id);
  ...
  for (auto&& [craned_id, task_ids] : running_task_craned_id_map) {
    g_thread_pool->detach_task(
        [id = craned_id, task_ids_to_cancel = task_ids]() {
          TASKSCHEDULER_TRACE("Craned {} is going to cancel tasks {}.", id,
...
void TaskScheduler::CleanSubmitQueueCb_() {
  ...
  do {
    ...
    if (!g_embedded_db_client->AppendTasksToPendingAndAdvanceTaskIds(
            accepted_task_ptrs)) {
      TASKSCHEDULER_ERROR("Failed to append a batch of tasks to embedded db queue.");
  ...
  do {
    ...
    TASKSCHEDULER_TRACE("Rejecting {} tasks...", rejected_actual_size);
...
void TaskScheduler::CleanTaskStatusChangeQueueCb_() {
  ...
  TASKSCHEDULER_TRACE("Cleaning {} TaskStatusChanges...", actual_size);
  ...
  for (const auto& [task_id, exit_code, new_status, craned_index] : args) {
    ...
    for (CranedId const& craned_id : task->CranedIds()) {
      ...
      if (node_to_task_map_it == m_node_to_tasks_map_.end()) [[unlikely]] {
        TASKSCHEDULER_ERROR("Failed to find craned_id {} in m_node_to_tasks_map_",
                    craned_id);
      } else {
    ...
    TASKSCHEDULER_TRACE("Move task#{} to the Completed Queue", task_id);
...
void MinLoadFirst::CalculateNodeSelectionInfoOfPartition_(
  ...
  for (const auto& craned_id : craned_ids) {
    ...
    if constexpr (kAlgoTraceOutput) {
      TASKSCHEDULER_TRACE("Craned node {} has running tasks: {}", craned_id,
    ...
    if constexpr (kAlgoTraceOutput) {
      TASKSCHEDULER_TRACE("Craned {} initial res_avail now: cpu: {}, mem: {}, gres: {}",
...
bool MinLoadFirst::CalculateRunningNodesAndStartTime_(
  ...
  while (available_craned_indexes_.size() < task->node_num &&
    ...
    if (!(task->requested_node_res_view <= craned_meta->res_total)) {
      ...
      if constexpr (kAlgoTraceOutput) {
        TASKSCHEDULER_TRACE("Task #{} excludes craned {}. Skipping this craned.",
                    task->TaskId(), craned_index);
      }
    } else {
  ...
  for (CranedId craned_id : craned_indexes_) {
    if constexpr (kAlgoTraceOutput) {
      TASKSCHEDULER_TRACE("Find valid time segments for task #{} on craned {}",
                  task->TaskId(), craned_id);
    }
    ...
    if (first_pass) {
      ...
      if constexpr (kAlgoTraceOutput) {
        ...
        TASKSCHEDULER_TRACE("After looping craned {}, valid time segments: {}",
...
void TaskScheduler::PersistAndTransferTasksToMongodb_(
  ...
  for (TaskInCtld* task : tasks) {
    if (!g_embedded_db_client->UpdateRuntimeAttrOfTask(txn_id, task->TaskDbId(),
                                                       task->RuntimeAttr()))
      TASKSCHEDULER_ERROR("Failed to call UpdateRuntimeAttrOfTask() for task #{}",
                  task->TaskId());
  }
  ...
  if (!g_db_client->InsertJobs(tasks)) {
    TASKSCHEDULER_ERROR("Failed to call g_db_client->InsertJobs() ");
    return;
  }
...
CraneErr TaskScheduler::AcquireTaskAttributes(TaskInCtld* task) {
  ...
  if (check_qos_result.has_error()) {
    TASKSCHEDULER_ERROR("Failed to call CheckAndApplyQosLimitOnTask: {}",
...
void TaskScheduler::TerminateTasksOnCraned(const CranedId& craned_id,
                                           uint32_t exit_code) {
  TASKSCHEDULER_TRACE("Terminate tasks on craned {}", craned_id);
  ...
  if (it != m_node_to_tasks_map_.end()) {
    ...
    TASKSCHEDULER_TRACE("No task is executed by craned {}. Ignore cleaning step...",
                craned_id);
  }
}

  ...
  if (std::filesystem::exists(config_path)) {
    try {
      ...
      if (config["CraneCtldForCranedListenPort"])
        g_config.ListenConf.CraneCtldForCranedListenPort = config["CraneCtldForCranedListenPort"].as<std::string>();
      else 
         g_config.ListenConf.CraneCtldForCranedListenPort = kCtldForCranedDefaultPort;

      if (config["CompressedRpc"])
        g_config.CompressedRpc = config["CompressedRpc"].as<bool>();

      if (config["UseTls"] && config["UseTls"].as<bool>()) {
        TlsCertificates& external_certs = g_config.ListenConf.TlsCerts.ExternalCerts;
        TlsCertificates& internal_certs = g_config.ListenConf.TlsCerts.InternalCerts;
        ClientTlsCertificates& craned_certs = g_config.ListenConf.TlsCerts.CranedClientCerts;
        ClientTlsCertificates& cfored_certs = g_config.ListenConf.TlsCerts.CforedClientCerts;

        g_config.ListenConf.UseTls = true;

        if (config["DomainSuffix"])
          g_config.ListenConf.TlsCerts.DomainSuffix = config["DomainSuffix"].as<std::string>();
        ...
        if (config["CranectldInternalCertFilePath"]) {
          ...
          if (internal_certs.ServerCertContent.empty()) {
            CRANE_ERROR(
                "UseTls is true, but the file specified by CranectldInternalCertFilePath "
                "is empty");
          }
        } else {
          CRANE_ERROR("UseTls is true, but CranectldInternalCertFilePath is empty");
          std::exit(1);
        }
        ...
        if (config["CranectldInternalKeyFilePath"]) {
          ...
          if (internal_certs.ServerKeyContent.empty()) {
            CRANE_ERROR(
                "UseTls is true, but the file specified by CranectldInternalKeyFilePath "
                "is empty");
          }
        } else {
          CRANE_ERROR("UseTls is true, but CranectldInternalKeyFilePath is empty");
          std::exit(1);
        }
...
void InitializeCtldGlobalVariables() {
  ...
  g_ctld_for_craned_server = std::make_unique<Ctld::CtldForCranedServer>(g_config.ListenConf);
}

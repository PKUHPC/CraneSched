...
void CranedMetaContainer::CranedUp(const CranedId& craned_id) {
  ...
  if (stub != nullptr && !stub->Invalid()) {
    ...
    if (err != CraneErr::kOk) {
      CRANE_ERROR("Default", "Failed to query actual resource from craned {}", craned_id);
      return;
    }
...
void CranedMetaContainer::MallocResourceFromNode(CranedId node_id,
  ...
  if (!craned_meta_map_.Contains(node_id)) {
    CRANE_ERROR("Default", "Try to malloc resource from an unknown craned {}", node_id);
    return;
  }
...
void CranedMetaContainer::FreeResourceFromNode(CranedId node_id,
                                               uint32_t task_id) {
  if (!craned_meta_map_.Contains(node_id)) {
    CRANE_ERROR("Default", "Try to free resource from an unknown craned {}", node_id);
    return;
  }
  ...
  auto node_meta = craned_meta_map_[node_id];
  if (!node_meta->alive) {
    CRANE_DEBUG("Default", "Crane {} has already been down. Ignore FreeResourceFromNode.",
  ...
  if (resource_iter == node_meta->running_task_resource_map.end()) {
    CRANE_ERROR("Default", "Try to free resource from an unknown task {} on craned {}",
                task_id, node_id);
    return;
  }
...
void CranedMetaContainer::InitFromConfig(const Config& config) {
  ...
  for (auto&& [craned_name, node_ptr] : config.Nodes) {
    CRANE_TRACE("Default", "Parsing node {}", craned_name);
  ...
  for (auto&& [part_name, partition] : config.Partitions) {
    CRANE_TRACE("Default", "Parsing partition {}", part_name);

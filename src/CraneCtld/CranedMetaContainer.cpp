...
void CranedMetaContainerSimpleImpl::InitFromConfig(const Config& config) {
  ...
  for (auto&& [craned_name, node_ptr] : config.Nodes) {
    ...
    static_meta.res.allocatable_resource.memory_sw_bytes =
        config.Nodes.at(craned_name)->memory_bytes;
    static_meta.res.dedicated_resource=config.Nodes.at(craned_name)->dedicated_resource;
  ...
  for (auto&& [part_name, partition] : config.Partitions) {
    ...
    for (auto&& craned_name : partition.nodes) {
      ...
      CRANE_DEBUG(
          "Add the resource of Craned {} (cpu: {}, mem: {}, gres{}) to partition "
          "[{}]'s global resource.",
          craned_name,
          ...
          util::ReadableGres(
              craned_meta.static_meta.res.dedicated_resource
          ),
          part_name);
    ...
    CRANE_DEBUG(
        "partition [{}]'s Global resource now: (cpu: {}, mem: {}, gres{}). It has {} "
        "craneds.",
        part_name,
...
CranedMetaContainerSimpleImpl::QueryAllPartitionInfo() {
  ...
  for (auto&& [part_name, part_meta] : partition_metas_map_) {
    ...
    auto& dedicated_res_total = part_meta.partition_global_meta.m_resource_total_inc_dead_.dedicated_resource;
    auto& dedicated_res_in_use = part_meta.partition_global_meta.m_resource_in_use_.dedicated_resource;
    auto& dedicated_res_avail = part_meta.partition_global_meta.m_resource_avail_.dedicated_resource;

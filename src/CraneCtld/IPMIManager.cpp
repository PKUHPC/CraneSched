...
bool IPMIManager::SleepNode(const CranedId& craned_id) {
  ...
  switch (node_info.state) {
    case NodeState::CranedRunning:
      if (ExecuteRemoteCommand_(craned_id, kSleepNodeCmd)) {
        node_info.state = NodeState::Sleeped;
        node_info.last_state_change = absl::Now();
        CRANE_DEBUG("Node {} entered sleep state via SSH", craned_id);
        return true;
      }
      
      CRANE_ERROR("Failed to put node {} to sleep", craned_id);
      return false;
...
bool IPMIManager::WakeupNode(const CranedId& craned_id) {
  ...
  switch (node_info.state) {
    case NodeState::Sleeped:
      if (ExecuteIPMICommand_(craned_id, kPowerOnCmd)) {
        node_info.state = NodeState::WakingUp;
        node_info.last_state_change = absl::Now();
        CRANE_DEBUG("Node {} is waking up via IPMI power on", craned_id);
        return true;
      }
...
bool IPMIManager::ShutdownNode(const CranedId& craned_id) {
  ...
  switch (node_info.state) {
    ...
    case NodeState::Sleeped:
      ...
      if (ExecuteIPMICommand_(craned_id, kPowerOffCmd)) {
  ...
  switch (node_info.state) {
    case NodeState::CranedRunning:
      if (!is_connected) {
        if (power_status == PowerState::Off) {
          ...
          CRANE_DEBUG("Node {} status changed to Shutdown from CranedRunning", craned_id);
        } else if (power_status == PowerState::On) {
          // Manual sleep
          node_info.state = NodeState::Sleeped;
          CRANE_DEBUG("Node {} status changed to Sleeped from CranedRunning", craned_id);
        } else {
          // Node abnormal
          node_info.state = NodeState::Unknown;
          CRANE_ERROR("Node {} status changed to Unknown from CranedRunning", craned_id);
        }
    ...
    case NodeState::Sleeped:
      if(is_connected) {
        ...
        node_info.last_state_change = now;
        CRANE_DEBUG("Node {} status changed to CranedRunning from Sleeped", craned_id);
      } else if(power_status == PowerState::Off) {
        // Manual power off
        node_info.state = NodeState::Shutdown;
        node_info.last_state_change = now;
        CRANE_DEBUG("Node {} status changed to Shutdown from Sleeped", craned_id);
      } else {
        CRANE_DEBUG("Node {} is still in sleep state", craned_id);
      }
      break;
    ...
    case NodeState::Shutdown:
      if(power_status == PowerState::On) {
        ...
        node_info.state = NodeState::PoweringUp;
        node_info.last_state_change = now;
        CRANE_DEBUG("Node {} status changed to PoweringUp from Shutdown", craned_id);
      } else {
        CRANE_DEBUG("Node {} is still in shutdown state", craned_id);
      }
      break;
    ...
    case NodeState::PoweringUp:
      {
        if (power_status == PowerState::On) {
          if(is_connected) {
            node_info.state = NodeState::CranedRunning;
            node_info.last_state_change = now;
            CRANE_DEBUG("Node {} powered up, Craned service is running", craned_id);
          } else {
            CRANE_ERROR("Node {} powered up, Craned service is not running", craned_id);
          }
        } else if (now - node_info.last_state_change > 
                   absl::Seconds(kPowerUpTimeoutSeconds)) {
          CRANE_ERROR("Node {} power up timeout, try to power up again", craned_id);
    ...
    case NodeState::WakingUp:
      if (is_connected) {
        ...
        CRANE_ERROR("Node {} wake up timeout, try to wake up again", craned_id);
    ...
    case NodeState::ShuttingDown:
      {
        if (power_status == PowerState::Off) {
          ...
          CRANE_ERROR("Node {} shutdown timeout, try to shutdown again", craned_id);
          ShutdownNode(craned_id);
        }
      }
      break;
...
bool IPMIManager::StartCranedService_(const CranedId& craned_id) {
  return ExecuteRemoteCommand_(craned_id, kStartCranedServiceCmd);
}

    ...
    if (!selected_nodes.empty()) {
      ...
      if (!nodes_to_wake.empty() || !nodes_to_power_on.empty()) {
        for (const CranedId& craned_id : nodes_to_wake) {
          if (IPMIManager::GetInstance().WakeupNode(craned_id)) {
        ...
        for (const CranedId& craned_id : nodes_to_power_on) {
          if (IPMIManager::GetInstance().PowerOnNode(craned_id)) {
...
double EnergyAwareNodeSelect::CalculateNodeScore_(
  ...
  CRANE_DEBUG(
      ...
      "{:.2f}W",
      info.recent_stats.current_power, info.historical_stats.avg_power);
  ...
  CRANE_DEBUG(
      "Score components - Power: {:.3f}, History: {:.3f}, Final: {:.3f}",
      power_score, history_score, final_score);
...
std::list<CranedId> EnergyAwareNodeSelect::SelectBestNodes_(
  ...
  running_candidates.reserve(energy_info.size());
  sleeped_candidates.reserve(energy_info.size());
  shutdown_candidates.reserve(energy_info.size());
  waking_candidates.reserve(energy_info.size());
  powering_candidates.reserve(energy_info.size());
  shutting_candidates.reserve(energy_info.size());

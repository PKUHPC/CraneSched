...
class SchedulerAlgo {
 ...
 private:
  ...
  template <typename UpdateNodeCostPolicy>
  class NodeSelector : public INodeSelector {
   public:
    struct NodeRater {
      NodeRater(const absl::Time& now, NodeState* node_state, double cost = 0.0)
          : node_state(node_state), cost(cost) {
        for (const auto& [start_time, end_time, res] :
             node_state->reserved_res) {
          UpdateNodeCostPolicy::UpdateCostFunc(cost, start_time, end_time, res,
                                               node_state->res_total);
        }

        for (const auto& [end_time, res] : node_state->allocated_res) {
          UpdateNodeCostPolicy::UpdateCostFunc(cost, now, end_time, res,
                                               node_state->res_total);
        }
      }
    ...
    void UpdateCost(const CranedId& craned_id, const absl::Time& start_time,
      ...
      UpdateNodeCostPolicy::UpdateCostFunc(node_info.cost, start_time, end_time,
                                           resources,
    ...
    void AddNode(const absl::Time& now, NodeState* node_state,
      ...
      NodeRater& node_info =
          m_node_info_map_
              .emplace(node_state->craned_id, NodeRater(now, node_state, cost))
              .first->second;
      node_info.cost_node_info_set_it =
          m_cost_node_info_set_.emplace(node_info.cost, node_info.node_state)
              .first;
  ...
  class LessCpuTimeRatioPolicy {
   public:
    static void UpdateCostFunc(double& cost, const absl::Time& start_time,
                               const absl::Time& end_time,
  ...
  class LocalScheduler {
   public:
    ...
    void InitializeNodeSelector(const absl::Time& now,
      ...
      m_node_selector_ = std::make_unique<NodeSelector<UpdateNodeCostPolicy>>();

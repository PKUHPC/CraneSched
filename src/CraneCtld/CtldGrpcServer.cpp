...
grpc::Status CraneCtldServiceImpl::QueryJobsInPartition(
  ...
  } else if (!request->set_task_ids().empty()) {
    for (const auto req : request->set_task_ids()) {
      response->set_ok(true);
      found = false;
      for (auto &task : task_list) {
        if (req == task.task_id) {
      ...
      if (!found) {
        response->set_ok(false);
        response->set_reason("Task id "+std::to_string(req)+" does not exist.");
      }
    }
  } else if (!request->set_partitions().empty()) {
    for (const auto &req : request->set_partitions()) {
      response->set_ok(true);
      found = false;
      for (auto &task : task_list) {
  ...
  } else if(!request->set_task_names().empty()){
    for (const auto &req : request->set_task_names()) {
      response->set_ok(true);
      found = false;
      for (auto &task : task_list) {
        if (req == task.name) {
  ...
  }else if (TaskStatus_IsValid(request->set_task_status())) {
    response->set_ok(true);
    for (auto &task : task_list) {
      if (request->set_task_status() != task.status) {
        continue;
      }
      auto *meta_it = meta_list->Add();
      meta_it->CopyFrom(task.task_to_ctld);
...
grpc::Status CraneCtldServiceImpl::QueryClusterInfo(
  ...
  *response = g_meta_container->QueryClusterInfo(
      request->set_query_partitions(),
      request->set_query_nodes(),
      request->set_query_states(),
      request->query_down_nodes(),
      request->query_responding_nodes());
  return grpc::Status::OK;
}

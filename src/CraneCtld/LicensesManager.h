...
class LicensesManager {
 public:
   ...
   bool CheckLicenseCountSufficient(std::unordered_map<LicenseId, uint32_t> &lic_id_to_count_map);

   void MallocLicenseResource(std::unordered_map<LicenseId, uint32_t> &lic_id_to_count_map);

   void FreeLicenseResource(std::unordered_map<LicenseId, uint32_t> &lic_id_to_count_map);

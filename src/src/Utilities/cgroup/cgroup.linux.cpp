...
bool Cgroup::SetDeviceLimit(uint64_t device_type,const std::vector<uint64_t>& device_index,bool allow,bool read,bool write,bool mknod){
  std::vector<std::string> strs(device_index.size());
  std::string op = (read ? "r" : "") + (write ? "w" : "") + (mknod ? "m" : "");
  char device_op_type = CgroupConstant::GetDeviceOpType(device_type);
  const std::string limit = std::format("{} {}:{{}} {}",device_op_type,CgroupConstant::GetDeviceId(device_type),op);
  for(int i = 0;uint64_t idx:device_index){
    strs[i]=std::format(limit,idx);
  }
  return SetControllerStrArray(CgroupConstant::Controller::DEVICES_CONTROLLER,
                              allow? ControllerFile::DEVICES_ALLOW:ControllerFile::DEVICES_DENY,
                              strs);
}

bool Cgroup::SetDeviceDeny(uint64_t device_type,const std::vector<uint64_t>& device_index){
...
bool Cgroup::SetControllerValue(CgroupConstant::Controller controller,
  ...
  if ((cg_controller = cgroup_get_controller(
    ...
    CRANE_WARN("Unable to get cgroup {} controller for {} {}.\n",
               CgroupConstant::GetControllerStringView(controller),
               m_cgroup_path_,cgroup_strerror(err));
    return false;
  }

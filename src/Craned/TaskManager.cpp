...
void TaskManager::InitDeviceBitmap(const DedicatedResource& dedicated_resource){
  for(const auto& entry:dedicated_resource.devices){
    m_device_bitmap_[entry.first]=(static_cast<uint64_t>(1)<<entry.second)-1;
  }
}

std::unordered_map<std::string,uint64_t> TaskManager::AllocateDeviceBitmap(uint32_t task_id,const DedicatedResource& dedicated_resource){
  ...
  for(const auto& entry:dedicated_resource.devices){
    ...
    for(int i = 0;i<DedicatedResource::BITMAPSIZE;i++){
      ...
      if(count == entry.second){
        ...
        alloc_device_bitmap[entry.first]=(avail_bitmap & ((static_cast<uint64_t>(1)<<(i+1))-1));
                         ...
                         std::to_string(instance->task.task_id()));
    {
      ...
      if(cuda_count!=0){
        env_vec.emplace_back("CUDA_VISIABLE_DEVICE",
                              util::GenerateCudaVisiableDeviceStr(
                                  cuda_count));
      }
    }
  ...
  while (this_->m_grpc_execute_task_queue_.try_dequeue(popped_instance)) {
    ...
    g_thread_pool->push_task([this_, instance]() {
      ...
      if (!DedicatedResourceAllocator::Allocate(
              this_->AllocateDeviceBitmap(instance->task.task_id(),DedicatedResource(instance->task.resources().dedicated_resource())),
              instance->cgroup)) {
        CRANE_ERROR(

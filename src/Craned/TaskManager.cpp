...
TaskManager::TaskManager() {
  ...
  if (m_ev_base_ == nullptr) {
    CRANE_ERROR("default", "Could not initialize libevent!");
    std::terminate();
  }
  {  // SIGCHLD
    m_ev_sigchld_ = evsignal_new(m_ev_base_, SIGCHLD, EvSigchldCb_, this);
    if (!m_ev_sigchld_) {
      CRANE_ERROR("default", "Failed to create the SIGCHLD event!");
      std::terminate();
    }

    if (event_add(m_ev_sigchld_, nullptr) < 0) {
      CRANE_ERROR("default", "Could not add the SIGCHLD event to base!");
      std::terminate();
    }
  }
  {
    ...
    if (!m_ev_process_sigchld_) {
      CRANE_ERROR("default", "Failed to create the Do SIGCHLD event!");
      std::terminate();
    }

    if (event_add(m_ev_process_sigchld_, nullptr) < 0) {
      CRANE_ERROR("default", "Could not add the Do SIGCHLD event to base!");
      std::terminate();
    }
  }
  {  // SIGINT
    ...
    if (event_add(m_ev_sigint_, nullptr) < 0) {
      CRANE_ERROR("default", "Could not add the SIGINT event to base!");
      std::terminate();
    }
  }
  {  // gRPC: QueryTaskIdFromPid
    ...
    if (!m_ev_query_task_id_from_pid_) {
      CRANE_ERROR("default", "Failed to create the query task id event!");
      std::terminate();
    }

    if (event_add(m_ev_query_task_id_from_pid_, nullptr) < 0) {
      CRANE_ERROR("default", "Could not add the query task id event to base!");
      std::terminate();
    }
  }
  {  // gRPC: QueryTaskEnvironmentVariable
    ...
    if (!m_ev_query_task_environment_variables_) {
      CRANE_ERROR("default", "Failed to create the query task env event!");
      std::terminate();
    }

    if (event_add(m_ev_query_task_environment_variables_, nullptr) < 0) {
      CRANE_ERROR("default", "Could not add the query task env to base!");
      std::terminate();
    }
  }
  {  // Exit Event
    ...
    if (!m_ev_exit_event_) {
      CRANE_ERROR("default", "Failed to create the exit event!");
      std::terminate();
    }

    if (event_add(m_ev_exit_event_, nullptr) < 0) {
      CRANE_ERROR("default", "Could not add the exit event to base!");
      std::terminate();
    }
  }
  {  // Grpc Execute Task Event
    ...
    if (!m_ev_grpc_execute_task_) {
      CRANE_ERROR("default", "Failed to create the grpc_execute_task event!");
      std::terminate();
    }
    if (event_add(m_ev_grpc_execute_task_, nullptr) < 0) {
      CRANE_ERROR("default", "Could not add the m_ev_grpc_execute_task_ to base!");
      std::terminate();
    }
  }
  {  // Task Status Change Event
    ...
    if (!m_ev_task_status_change_) {
      CRANE_ERROR("default", "Failed to create the task_status_change event!");
      std::terminate();
    }
    if (event_add(m_ev_task_status_change_, nullptr) < 0) {
      CRANE_ERROR("default", "Could not add the m_ev_task_status_change_event_ to base!");
      std::terminate();
    }
  }
  {
    ...
    if (!m_ev_task_time_limit_change_) {
      CRANE_ERROR("default", "Failed to create the task_time_limit_change event!");
      std::terminate();
    }
    if (event_add(m_ev_task_time_limit_change_, nullptr) < 0) {
      CRANE_ERROR("default", "Could not add the m_ev_task_time_limit_change_ to base!");
      std::terminate();
    }
  }
  {
    ...
    if (!m_ev_task_terminate_) {
      CRANE_ERROR("default", "Failed to create the task_terminate event!");
      std::terminate();
    }
    if (event_add(m_ev_task_terminate_, nullptr) < 0) {
      CRANE_ERROR("default", "Could not add the m_ev_task_terminate_ to base!");
      std::terminate();
    }
  }
  {
    ...
    if (!m_ev_check_task_status_) {
      CRANE_ERROR("default", "Failed to create the check_task_status event!");
      std::terminate();
    }
    if (event_add(m_ev_check_task_status_, nullptr) < 0) {
      CRANE_ERROR("default", "Could not add the m_ev_check_task_status_ to base!");
      std::terminate();
    }
  }
...
void TaskManager::TaskStopAndDoStatusChangeAsync(uint32_t task_id) {
  ...
  if (it == m_task_map_.end()) {
    CRANE_ERROR("default", "Task #{} not found in TaskStopAndDoStatusChangeAsync.",
                task_id);
    return;
  }
  TaskInstance* instance = it->second.get();

  CRANE_INFO("default", "Task #{} stopped and is doing TaskStatusChange...", task_id);
  ...
  while (true) {
    ...
    if (pid > 0) {
      ...
      if (WIFEXITED(status)) {
        ...
        CRANE_TRACE("default", "Receiving SIGCHLD for pid {}. Signaled: false, Status: {}",
        ...
        CRANE_TRACE("default", "Receiving SIGCHLD for pid {}. Signaled: true, Signal: {}",
                    pid, WTERMSIG(status));
      }
      ...
      if (errno != ECHILD)
        CRANE_DEBUG("default", "waitpid() error: {}, {}", errno, strerror(errno));
      break;
    }
  }
}
...
void TaskManager::EvProcessSigchldCb_(int sig, short events, void* user_data) {
  ...
  while (this_->m_sigchld_queue_.try_dequeue(sigchld_info)) {
    ...
    if (task_iter == this_->m_pid_task_map_.end() ||
      ...
      CRANE_TRACE("default", "Child Process {} exit too early, will do SigchldCb later",
                  sigchld_info->pid);
    ...
    if (pr_it == instance->processes.end()) {
      CRANE_ERROR("default", "Failed to find pid {} in task #{}'s ProcessInstances", pid,
                  task_id);
    } else {
...
void TaskManager::EvSubprocessReadCb_(struct bufferevent* bev, void* process) {
  ...
  CRANE_TRACE("default", "Read {:>4} bytes from subprocess (pid: {}): {}", n_copy,
              proc->GetPid(), str);
...
void TaskManager::EvSigintCb_(int sig, short events, void* user_data) {
  ...
  if (!this_->m_is_ending_now_) {
    ...
    CRANE_INFO("default", "Caught SIGINT. Send SIGTERM to all running tasks...");
    ...
    for (auto task_it = this_->m_task_map_.begin();
      ...
      if (task_instance->task.type() == crane::grpc::Batch ||
        ...
        // task map.
        CRANE_DEBUG("default", "Cleaning Calloc task #{}...",
...
void TaskManager::EvExitEventCb_(int efd, short events, void* user_data) {
  ...
  CRANE_TRACE("default", "Exit event triggered. Stop event loop.");
...
void TaskManager::EvActivateShutdown_() {
  CRANE_TRACE("default", "Triggering exit event...");
...
CraneErr TaskManager::KillProcessInstance_(const ProcessInstance* proc,
  ...
  if (proc) {
    CRANE_TRACE("default", "Killing pid {} with signal {}", proc->GetPid(), signum);
    ...
    if (err == 0)
      ...
      CRANE_TRACE("default", "kill failed. error: {}", strerror(errno));
...
CraneErr TaskManager::SpawnProcessInInstance_(TaskInstance* instance,
  ...
  if (!res_in_node.has_value()) {
    CRANE_ERROR("default", "Failed to get resource info for task #{}",
                instance->task.task_id());
    return CraneErr::kCgroupError;
  }

  if (socketpair(AF_UNIX, SOCK_STREAM, 0, ctrl_sock_pair) != 0) {
    CRANE_ERROR("default", "Failed to create socket pair: {}", strerror(errno));
  ...
  if (instance->IsCrun()) {
    if (socketpair(AF_UNIX, SOCK_STREAM, 0, io_in_sock_pair) != 0) {
      CRANE_ERROR("default", "Failed to create socket pair for task io forward: {}",
                  strerror(errno));
      return CraneErr::kSystemErr;
    }
  ...
  if (rc == -1) {
    CRANE_ERROR("default", "error: setegid. {}", strerror(errno));
    return CraneErr::kSystemErr;
  }
  ...
  if (rc == -1) {
    CRANE_ERROR("default", "error: seteuid. {}", strerror(errno));
    return CraneErr::kSystemErr;
  }

  pid_t child_pid = fork();
  if (child_pid == -1) {
    CRANE_ERROR("default", "fork() failed for task #{}: {}", instance->task.task_id(),
                strerror(errno));
    return CraneErr::kSystemErr;
  }
  ...
  if (child_pid > 0) {  // Parent proc
    ...
    CRANE_DEBUG("default", "Subprocess was created for task #{} pid: {}",
    ...
    CRANE_TRACE("default", "New task #{} is ready. Asking subprocess to execv...",
                instance->task.task_id());
    ...
    if (!ok) {
      CRANE_ERROR("default", "Failed to serialize msg to ostream: {}",
                  strerror(ostream.GetErrno()));
    }

    if (ok) ok &= ostream.Flush();
    if (!ok) {
      CRANE_ERROR("default", "Failed to send ok=true to subprocess {} for task #{}: {}",
    ...
    if (!ok || !msg.ok()) {
      if (!ok)
        CRANE_ERROR("default", "Socket child endpoint failed: {}",
                    strerror(istream.GetErrno()));
      if (!msg.ok())
        CRANE_ERROR("default", "False from subprocess {} of task #{}", child_pid,
                    instance->task.task_id());
      close(ctrl_fd);
  ...
  AskChildToSuicide:
    ...
    close(ctrl_fd);
    if (!ok) {
      CRANE_ERROR("default", "Failed to ask subprocess {} to suicide for task #{}",
                  child_pid, instance->task.task_id());
    ...
    rc = chdir(cwd.c_str());
    if (rc == -1) {
      CRANE_ERROR("default", "[Child Process] Error: chdir to {}. {}", cwd.c_str(),
                  strerror(errno));
      std::abort();
    }
    ...
    if (!ok || !msg.ok()) {
      if (!ok) {
        ...
        CRANE_ERROR("default", "Failed to read socket from parent: {}", strerror(err));
      }

      if (!msg.ok())
        CRANE_ERROR("default", "Parent process ask not to start the subprocess.");
    ...
    if (instance->task.type() == crane::grpc::Batch) {
      ...
      if (stdout_fd == -1) {
        CRANE_ERROR("default", "[Child Process] Error: open {}. {}", stdout_file_path,
                    strerror(errno));
        std::abort();
      }
      dup2(stdout_fd, 1);
      ...
      if (stderr_file_path.empty()) {
        ...
        if (stderr_fd == -1) {
          CRANE_ERROR("default", "[Child Process] Error: open {}. {}", stderr_file_path,
                      strerror(errno));
          std::abort();
        }
    ...
    ok &= ostream.Flush();
    if (!ok) {
      CRANE_ERROR("default", "[Child Process] Error: Failed to flush.");
      std::abort();
    }
...
CraneErr TaskManager::ExecuteTaskAsync(crane::grpc::TaskToD const& task) {
  if (!g_cg_mgr->CheckIfCgroupForTasksExists(task.task_id())) {
    CRANE_DEBUG("default", "Executing task #{} without an allocated cgroup. Ignoring it.",
                task.task_id());
    return CraneErr::kCgroupError;
  }
  CRANE_INFO("default", "Executing task #{}", task.task_id());
...
void TaskManager::EvGrpcExecuteTaskCb_(int, short events, void* user_data) {
  ...
  while (this_->m_grpc_execute_task_queue_.try_dequeue(popped_instance)) {
    ...
    if (!ok) {
      CRANE_ERROR("default", "Duplicated ExecuteTask request for task #{}. Ignore it.",
                  task_id);
      continue;
    }
    ...
    CRANE_TRACE("default", "Add a timer of {} seconds for task #{}", sec, task_id);
...
void TaskManager::LaunchTaskInstanceMt_(TaskInstance* instance) {
  ...
  if (!g_cg_mgr->CheckIfCgroupForTasksExists(task_id)) {
    CRANE_ERROR("default", "Failed to find created cgroup for task #{}", task_id);
    EvActivateTaskStatusChange_(
        task_id, crane::grpc::TaskStatus::Failed,
  ...
  if (!instance->pwd_entry.Valid()) {
    CRANE_DEBUG("default", "Failed to look up password entry for uid {} of task #{}",
  ...
  if (!ok) {
    CRANE_ERROR("default", "Failed to allocate cgroup for task #{}", task_id);
    EvActivateTaskStatusChange_(
        task_id, crane::grpc::TaskStatus::Failed,
        ExitCode::kExitCodeCgroupError,
  ...
  if (fptr == nullptr) {
    CRANE_ERROR("default", "Failed write the script for task #{}", task_id);
    EvActivateTaskStatusChange_(
        task_id, crane::grpc::TaskStatus::Failed,
...
void TaskManager::EvOnTaskTimerCb_(int, short, void* arg_) {
  ...
  CRANE_TRACE("default", "Task #{} exceeded its time limit. Terminating it...", task_id);
  ...
  if (task_it == this_->m_task_map_.end()) {
    CRANE_TRACE("default", "Task #{} has already been removed.");
    return;
  }
...
void TaskManager::EvTerminateTaskCb_(int efd, short events, void* user_data) {
  ...
  while (this_->m_task_terminate_queue_.try_dequeue(elem)) {
    ...
    if (iter == this_->m_task_map_.end()) {
      CRANE_DEBUG("default", "Terminating a non-existent task #{}.", elem.task_id);
...
void TaskManager::EvChangeTaskTimeLimitCb_(int, short events, void* user_data) {
  ...
  while (this_->m_task_time_limit_change_queue_.try_dequeue(elem)) {
    ...
    if (iter != this_->m_task_map_.end()) {
      ...
      CRANE_ERROR("default", "Try to update the time limit of a non-existent task #{}.",
                  elem.task_id);

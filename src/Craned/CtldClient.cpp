...
CtldClient::~CtldClient() {
  ...
  CRANE_TRACE("Default", "CtldClient is ending. Waiting for the thread to finish.");
...
void CtldClient::OnCraneCtldConnected() {
  ...
  CRANE_INFO("Default", "Send a register RPC to cranectld");
    ...
    if (!status.ok()) {
      ...
      if (reply.ok()) {
        if (reply.already_registered()) {
          CRANE_INFO("Default", "This craned has already registered.");
          return;
        } else {
          CRANE_INFO("Default", 
        ...
        CRANE_ERROR("Default", "This Craned is not allow to register.");
        return;
      }
    }
  } while (retry_time--);

  CRANE_ERROR("Default", "Failed to register actively.");
}
...
void CtldClient::AsyncSendThread_() {
  ...
  while (true) {
    ...
    if (!connected) {
      CRANE_INFO("Default", "Channel to CraneCtlD is not connected. Reconnecting...");
    ...
    while (!changes.empty()) {
      ...
      CRANE_TRACE("Default", "Sending TaskStatusChange for task #{}",
      ...
      if (!status.ok()) {
        ...
        CRANE_TRACE("Default", "TaskStatusChange for task #{} sent. reply.ok={}",

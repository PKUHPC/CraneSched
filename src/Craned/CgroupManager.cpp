...
int CgroupManager::Init() {
  ...
  CRANED_DEBUG("Initializing cgroup library.");
  cgroup_init();
  ...
  if (!Mounted(Controller::BLOCK_CONTROLLER)) {
    CRANED_WARN("Cgroup controller for I/O statistics is not available.\n");
  }
  if (!Mounted(Controller::FREEZE_CONTROLLER)) {
    CRANED_WARN("Cgroup controller for process management is not available.\n");
  }
  if (!Mounted(Controller::CPUACCT_CONTROLLER)) {
    CRANED_WARN("Cgroup controller for CPU accounting is not available.\n");
  }
  if (!Mounted(Controller::MEMORY_CONTROLLER)) {
    CRANED_WARN("Cgroup controller for memory accounting is not available.\n");
  }
  if (!Mounted(Controller::CPU_CONTROLLER)) {
    CRANED_WARN("Cgroup controller for CPU is not available.\n");
  }
  if (!Mounted(Controller::DEVICES_CONTROLLER)) {
    CRANED_WARN("Cgroup controller for DEVICES is not available.\n");
  }
  if (ret != ECGEOF) {
    CRANED_WARN("Error iterating through cgroups mount information: {}\n",
               cgroup_strerror(ret));
    return -1;
  }
...
int CgroupManager::InitializeController_(struct cgroup &cgroup,
  ...
  if (!Mounted(controller)) {
    if (required) {
      CRANED_WARN("Error - cgroup controller {} not mounted, but required.\n",
                 CgroupConstant::GetControllerStringView(controller));
      return 1;
    } else {
  ...
  if (!has_cgroup ||
    ...
    if ((p_raw_controller = cgroup_add_controller(
      ...
      CRANED_WARN("Unable to initialize cgroup {} controller.\n",
                 controller_str);
      return required ? 1 : 0;
    } else {
      ...
      if (controller == CgroupConstant::Controller::MEMORY_CONTROLLER) {
        if ((err = cgroup_add_value_bool(p_raw_controller,
          ...
          CRANED_WARN("Unable to set hierarchical memory settings: {} {}\n", err,
                     cgroup_strerror(err));
        }
      }
    }
...
std::unique_ptr<Cgroup> CgroupManager::CreateOrOpen_(
  ...
  if (native_cgroup == NULL) {
    CRANED_WARN("Unable to construct new cgroup object.\n");
    return nullptr;
  }
...
bool CgroupManager::CreateCgroups(std::vector<CgroupSpec> &&cg_specs) {
  ...
  CRANED_DEBUG("Creating cgroups for {} tasks", cg_specs.size());
  ...
  for (int i = 0; i < cg_specs.size(); i++) {
    ...
    CRANED_TRACE("Create lazily allocated cgroups for task #{}, uid {}", task_id,
                uid);
  ...
  CRANED_TRACE("Create cgroups costed {} ms",
...
void CgroupManager::RmAllTaskCgroupsUnderController_(
  ...
  while (ret == 0) {
    if (info.type == cgroup_file_type::CGROUP_FILE_TYPE_DIR &&
      ...
      CRANED_DEBUG("Removing remaining task cgroup: {}", info.full_path);
      int err = rmdir(info.full_path);
      if (err != 0)
        CRANED_ERROR("Failed to remove cgroup {}: {}", info.full_path,
                    strerror(errno));
    }
...
bool CgroupManager::QueryTaskInfoOfUidAsync(uid_t uid, TaskInfoOfUid *info) {
  CRANED_DEBUG("Query task info for uid {}", uid);
...
std::vector<EnvPair> CgroupManager::GetResourceEnvListOfTask(
  ...
  if (cg_spec_ptr)
    ...
    CRANED_ERROR("Trying to get resource env list of a non-existent task #{}",
                task_id);
...
//      CRANED_WARN("Unable to read original cgroup {}. Error {}: {}\n",
...
after_migrate:
  ...
  if (err != 0) {
    CRANED_WARN("Cannot attach pid {} to cgroup {}: {} {}", pid,
...
//  if (cpu_cg_f == nullptr) {
//    CRANED_ERROR("fopen failed: {}", strerror(errno));
//    err = 1;
//    goto end;
//  } else {
//    CRANED_TRACE("Open {} succeeded.", cpu_cg_path);
//  }
//
//  err = fprintf(cpu_cg_f, "%d", pid);
//  if (err < 0) {
//    CRANED_ERROR("fprintf failed: {}", strerror(errno));
//    goto end;
//  } else {
//    CRANED_TRACE("fprintf {} bytes succeeded.", err);
//  }
//
//  err = fflush(cpu_cg_f);
//  if (err < 0) {
//    CRANED_ERROR("fflush failed: {}", strerror(errno));
//    goto end;
//  } else {
//    CRANED_TRACE("fflush succeeded.");
//  }
//
//  fclose(cpu_cg_f);
//
...
//      CRANED_TRACE("Pid in {}: {}", cgroup_path, line);
//    }
...
Cgroup::~Cgroup() {
  if (m_cgroup_) {
    ...
    if ((err = cgroup_delete_cgroup_ext(
      ...
      CRANED_ERROR("Unable to completely remove cgroup {}: {} {}\n",
                  m_cgroup_path_.c_str(), err, cgroup_strerror(err));
    }
...
bool Cgroup::SetControllerValue(CgroupConstant::Controller controller,
  ...
  if (!g_cg_mgr->Mounted(controller)) {
    CRANED_ERROR("Unable to set {} because cgroup {} is not mounted.",
  ...
  if ((cg_controller = cgroup_get_controller(
           ...
           CgroupConstant::GetControllerStringView(controller).data())) ==
      nullptr) {
    CRANED_ERROR("Unable to get cgroup {} controller for {}.",
                CgroupConstant::GetControllerStringView(controller),
                m_cgroup_path_);
    return false;
  }
  ...
  if ((err = cgroup_set_value_uint64(
           ...
           value))) {
    CRANED_ERROR("Unable to set uint64 value for {} in cgroup {}. Code {}, {}",
                CgroupConstant::GetControllerFileStringView(controller_file),
...
bool Cgroup::SetControllerStr(CgroupConstant::Controller controller,
  ...
  if ((err = cgroup_set_value_string(
           ...
           CgroupConstant::GetControllerFileStringView(controller_file).data(),
           str.c_str()))) {
    CRANED_ERROR("Unable to set string for {}: {} {}\n", m_cgroup_path_, err,
                cgroup_strerror(err));
    return false;
  }
...
bool Cgroup::ModifyCgroup_(CgroupConstant::ControllerFile controller_file) {
  ...
  while (true) {
    ...
    if (err != ECGOTHER) {
      CRANED_ERROR("Unable to modify_cgroup for {} in cgroup {}. Code {}, {}",
                  CgroupConstant::GetControllerFileStringView(controller_file),
                  m_cgroup_path_, err, cgroup_strerror(err));
      return false;
    }
    ...
    retry_time++;
    if (retry_time > 3) {
      CRANED_ERROR("Unable to modify_cgroup for cgroup {} after 3 times.",
                  m_cgroup_path_);
      return false;
    }
  }
...
bool Cgroup::SetControllerStrs(CgroupConstant::Controller controller,
  ...
  if ((cg_controller = cgroup_get_controller(
           ...
           CgroupConstant::GetControllerStringView(controller).data())) ==
      nullptr) {
    CRANED_WARN("Unable to get cgroup {} controller for {}.\n",
               CgroupConstant::GetControllerStringView(controller),
               m_cgroup_path_);
    return false;
  }
  for (const auto &str : strs) {
    if ((err = cgroup_set_value_string(
             ...
             CgroupConstant::GetControllerFileStringView(controller_file)
                 .data(),
             str.c_str()))) {
      CRANED_WARN("Unable to add string for {}: {} {}\n", m_cgroup_path_, err,
                 cgroup_strerror(err));
      return false;
    }
    // Commit cgroup modifications.
    if ((err = cgroup_modify_cgroup(m_cgroup_))) {
      CRANED_WARN("Unable to commit {} for cgroup {}: {} {}\n",
                 CgroupConstant::GetControllerFileStringView(controller_file),
                 m_cgroup_path_, err, cgroup_strerror(err));
      return false;
    }
...
bool Cgroup::KillAllProcesses() {
  ...
  if (rc == 0) {
    ...
    CRANED_ERROR("cgroup_get_procs error on cgroup \"{}\": {}", cg_name,
                cgroup_strerror(rc));
    return false;
  }
}

...
void InitFromStdin(int argc, char** argv) {
  ...
  g_config.CforedListenConf.TlsCerts.CertContent = msg.cfored_listen_conf().tls_certs().cert_content();
  g_config.CforedListenConf.TlsCerts.CaContent = msg.cfored_listen_conf().tls_certs().ca_content();
  g_config.CforedListenConf.TlsCerts.KeyContent = msg.cfored_listen_conf().tls_certs().key_content();
  g_config.CforedListenConf.DomainSuffix = msg.cfored_listen_conf().domain_suffix();

        ...
        for (auto it = config["Nodes"].begin(); it != config["Nodes"].end();
          ...
          if (node["gres"]){
            ...
            while(std::regex_search(gres,entry,pattern)){
              ...
              std::string device_type = entry[2].matched ? std::string(entry[2]):std::string("default");
              uint64_t device_num = std::stoul(entry[3]);
              if(entry[2].matched){
                gres_conf.AddResource(fmt::format("{}:{}",device_name,device_type),device_num);
              }else{
                gres_conf.AddResource(device_name,device_num);
              }
              CRANE_INFO("Gres conf parse:gres_name: `{}` ,gres_type: `{}` ,gres_num: `{}` ",

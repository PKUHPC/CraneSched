  ...
  if (std::filesystem::exists(config_path)) {
    try {
      ...
      if (config["UseTls"] && config["UseTls"].as<bool>()) {
        ...
        TlsCertificates& craned_certs = g_config.ListenConf.TlsCerts.CranedTlsCerts;
        ClientTlsCertificates& internal_client_certs = g_config.ListenConf.TlsCerts.InternalClientTlsCerts;
        ...
        if (config["CranectldInternalCertFilePath"]) {
          ...
          try {
            internal_client_certs.ClientCertContent =
                util::ReadFileIntoString(internal_client_certs.ClientCertFilePath);
          } catch (const std::exception& e) {
            CRANE_ERROR("Read cert file error: {}", e.what());
            std::exit(1);
          }
      ...
      if (config["CraneCtldForCranedListenPort"])
        g_config.CraneCtldForCranedPort = config["CraneCtldForCranedListenPort"].as<std::string>();
      else

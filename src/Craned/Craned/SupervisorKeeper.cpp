...
#include <protos/Supervisor.grpc.pb.h>
  ...
  CraneExpected<EnvMap> SupervisorClient::QueryStepEnv() {
    ClientContext context;
  ...
  CraneExpected<std::pair<task_id_t, pid_t>>
  SupervisorClient::CheckTaskStatus() {
    ClientContext context;
  ...
  CraneErrCode SupervisorClient::ChangeTaskTimeLimit(
  ...
  void SupervisorKeeper::AddSupervisor(task_id_t task_id) {
    ...
    if (auto it = m_supervisor_map.find(task_id);
        it != m_supervisor_map.end()) {
  ...
  std::shared_ptr<SupervisorClient> SupervisorKeeper::GetStub(
      task_id_t task_id) {

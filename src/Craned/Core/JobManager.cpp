...
CraneErrCode JobManager::SpawnSupervisor_(JobInD* job, StepInstance* step) {
  ...
  if (child_pid > 0) {  // Parent proc
    ...
    cfored_listen_conf->set_use_tls(g_config.ListenConf.UseTls);
    cfored_listen_conf->set_domain_suffix(g_config.ListenConf.DomainSuffix);
    auto* tls_certs = cfored_listen_conf->mutable_tls_certs();
    tls_certs->set_cert_content(g_config.ListenConf.TlsCerts.CertContent);
    tls_certs->set_ca_content(g_config.ListenConf.TlsCerts.CaContent);
    tls_certs->set_key_content(g_config.ListenConf.TlsCerts.KeyContent);

...
Config::HealthCheckConfig::NodeStateEnum JobManager::GetNodeState() {
  auto job_map_ptr = m_job_map_.GetMapConstSharedPtr();
  if (!job_map_ptr || job_map_ptr->empty()) return Config::HealthCheckConfig::NodeStateEnum::IDLE;
  ...
  if (res_in_use.IsZero()) return Config::HealthCheckConfig::NodeStateEnum::IDLE;

  ResourceInNode res_avail = (*g_config.CranedRes[g_config.CranedIdOfThisNode]);
  res_avail -= res_in_use;
  if (res_avail.allocatable_res.IsAnyZero()) return Config::HealthCheckConfig::NodeStateEnum::ALLOC;

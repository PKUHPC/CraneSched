...
CforedClient::~CforedClient() {
  CRANED_TRACE("CforedClient to {} is being destructed.", m_cfored_name_);
  m_stopped_ = true;
  if (m_fwd_thread_.joinable()) m_fwd_thread_.join();
  m_cq_.Shutdown();
  CRANED_TRACE("CforedClient to {} was destructed.", m_cfored_name_);
};
...
void CforedClient::CleanOutputQueueAndWriteToStreamThread_(
  ...
  CRANED_TRACE("CleanOutputQueueThread started.");
  ...
  CRANED_TRACE("CleanOutputQueueThread exited.");
}
...
void CforedClient::AsyncSendRecvThread_() {
  ...
  CRANED_TRACE("Preparing TaskIOStream...");
  ...
  while (true) {
    ...
    // CRANED_TRACE("NextStatus: {}, ok: {}, Tag received: {}, state: {}",
    ...
    if (next_status == grpc::CompletionQueue::TIMEOUT) {
      if (m_stopped_) {
        CRANED_TRACE("TIMEOUT with m_stopped_=true.");
        ...
        CRANED_TRACE("Unregistering on cfored {}.", m_cfored_name_);
    ...
    if (!ok && tag != Tag::Prepare) {
      CRANED_ERROR("Cfored connection failed.");
      state = State::End;
    }
    ...
    case State::Registering:
      ...
      CRANED_TRACE("Registering new stream on cfored {}", m_cfored_name_);
    ...
    case State::WaitRegisterAck: {
      CRANED_TRACE("WaitRegisterAck");

      if (tag == Tag::Write) {
        write_pending.store(false, std::memory_order::release);
        CRANED_TRACE("Cfored Registration was sent. Reading Ack...");

        reply.Clear();
        stream->Read(&reply, (void*)Tag::Read);
      } else if (tag == Tag::Read) {
        CRANED_TRACE("Cfored RegisterAck Read. Start Forwarding..");
    ...
    case State::Forwarding: {
      ...
      if (reply.type() != StreamCforedTaskIOReply::CRANED_TASK_INPUT) {
        CRANED_ERROR("Expect TASK_INPUT, but got {}", (int)reply.type());
        break;
      }
      ...
      if (fwd_meta_it != m_task_fwd_meta_map_.end()) {
        ...
        CRANED_ERROR("Cfored {} trying to send msg to unknown task #{}",
    ...
    case State::Unregistering:
      if (tag == Tag::Write) {
        CRANED_TRACE("UNREGISTER msg was sent. waiting for reply...");
        break;
      }
      CRANE_ASSERT(tag == Tag::Read);
      CRANED_TRACE("UNREGISTER_REPLY msg received.");

      if (reply.type() != StreamCforedTaskIOReply::CRANED_UNREGISTER_REPLY) {
        CRANED_TRACE("Expect UNREGISTER_REPLY, but got {}. Ignoring it.",
                    reply.type());
        reply.Clear();
        stream->Read(&reply, (void*)Tag::Read);
        break;
    ...
    CRANED_TRACE("Next state: {}", int(state));
...
void CforedClient::TaskOutPutForward(task_id_t task_id,
  ...
  CRANED_TRACE("Receive TaskOutputForward for task #{}: {}", task_id, msg);
...
CforedManager::~CforedManager() {
  CRANED_TRACE("CforedManager destructor called.");
  m_stopped_ = true;
...
void CforedManager::EvLoopThread_(const std::shared_ptr<uvw::loop>& uvw_loop) {
  ...
  if (idle_handle->start() != 0) {
    CRANED_ERROR("Failed to start the idle event in CforedManager EvLoop.");
  }
...
void CforedManager::RegisterCb_() {
  ...
  while (m_register_queue_.try_dequeue(p)) {
    ...
    m_cfored_client_map_[elem.cfored]->InitTaskFwdAndSetInputCb(
        elem.task_id, [fd = elem.in_fd](const std::string& msg) -> bool {
          ...
          while (sz_sent != msg.size()) {
            ...
            if (sz_written < 0) {
              CRANED_ERROR("Pipe to Crun task was broken.");
              return false;
            }
    ...
    CRANED_TRACE("Registering fd {} for outputs of task #{}", elem.out_fd,
                elem.task_id);
    ...
    poll_handle->on<uvw::poll_event>([this, elem = std::move(elem)](
                                         ...
                                         uvw::poll_handle& h) {
      CRANED_TRACE("Detect task #{} output.", elem.task_id);
      ...
      if (ret == 0) {
        CRANED_TRACE("Task #{} to cfored {} finished its output.", elem.task_id,
                    elem.cfored);
        h.close();
        close(elem.out_fd);
        ...
        if (ok_to_free) {
          CRANED_TRACE("It's ok to unregister task #{} on {}", elem.task_id,
                      elem.cfored);
      ...
      if (ret == -1)
        CRANED_ERROR("Error when reading task #{} output", elem.task_id);

      std::string output(buf, ret);
      CRANED_TRACE("Fwd to task #{}: {}", elem.task_id, output);
    ...
    if (ret < 0)
      CRANED_ERROR("poll_handle->start() error: {}", uv_strerror(ret));
...
void CforedManager::TaskStopCb_() {
  ...
  while (m_task_stop_queue_.try_dequeue(elem)) {
    ...
    CRANED_TRACE("Task #{} to cfored {} just stopped its process.", elem.task_id,
                elem.cfored);
    bool ok_to_free =

...
class TaskManager {
 ...
 private:
  ...
  std::unordered_map<std::string,uint64_t> AllocateDeviceBitmap(uint32_t task_id,const DedicatedResource& dedicated_resource);
  ...
  std::unordered_map<uint32_t,std::unordered_map<std::string,uint64_t>> m_task_id_device_bitmap_;
  absl::Mutex m_mtx_;

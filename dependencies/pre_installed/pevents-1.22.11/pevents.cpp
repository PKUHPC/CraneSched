...
namespace neosmart {
#  ifdef WFMO
// Each call to WaitForMultipleObjects initializes a neosmart_wfmo_t object
// which tracks the progress of the caller's multi-object wait and dispatches
// responses accordingly. One neosmart_wfmo_t struct is shared for all events in
// a single WFMO call
...
// A neosmart_wfmo_info_t object is registered with each event waited on in a
// WFMO This reference to neosmart_wfmo_t_ is how the event knows whom to notify
// when triggered
...
// The basic event structure, passed to the caller as an opaque pointer when
// creating events
...
neosmart_event_t CreateEvent(bool manualReset, bool initialState) {
  ...
  // memory_order_release: if `initialState == true`, allow a load with acquire
  // semantics to see the value.
...
static int UnlockedWaitForEvent(neosmart_event_t event, uint64_t milliseconds) {
  int result = 0;
  // memory_order_relaxed: `State` is only set to true with the mutex held, and
  // we require that this function only be called after the mutex is obtained.
  if (!event->State.load(std::memory_order_relaxed)) {
    ...
    if (milliseconds != WAIT_INFINITE) {
      ...
      ts.tv_nsec =
          (long)(nanoseconds - ((uint64_t)ts.tv_sec) * 1000 * 1000 * 1000);
    }
    ...
    do {
      ...
      // memory_order_relaxed: ordering is guaranteed by the mutex, as `State =
      // true` is only ever written with the mutex held.
    ...
    // We're trying to obtain a manual reset event with a signaled state; don't
    // do anything
  }

  if (result == 0 && event->AutoReset) {
    // We've only accquired the event if the wait succeeded
    // memory_order_relaxed: we never act on `State == true` without fully
    // synchronizing or grabbing the mutex, so it's OK to use relaxed semantics
    // here.
...
int WaitForEvent(neosmart_event_t event, uint64_t milliseconds) {
  // Optimization: bypass acquiring the event lock if the state atomic is
  // unavailable. memory_order_relaxed: This is just an optimization, it's OK to
  // be biased towards a stale value here, and preferable to synchronizing CPU
  // caches to get a more accurate result.
...
int WaitForMultipleEvents(neosmart_event_t *events, int count, bool waitAll,
  ...
  // Separately keep track of how many refs to decrement after the
  // initialization loop, to avoid repeatedly clearing the cache line.
  int skipped_refs = 0;
  ...
  for (int i = 0; i < count; ++i) {
    ...
    // Skip obtaining the mutex for manual reset events. This requires a memory
    ...
    if (!skipLock) {
      ...
      // Before adding this wait to the list of registered waits, let's clean up
      // old, expired waits while we have the event lock anyway.
      events[i]->RegisteredWaits.erase(
          std::remove_if(events[i]->RegisteredWaits.begin(),
  ...
  // We set the `done` flag above in case of WaitAny and at least one event was
  // set. But we need to check again here if we were doing a WaitAll or else
  // we'll incorrectly return WAIT_TIMEOUT.
  ...
  while (!done) {
    ...
    // If we're waiting for all events, assume we're done and check if there's
    // an event that hasn't fired But if we're waiting for just one event,
    // assume we're not done until we find a fired event
  ...
  // memory_order_relaxed: this is only checked outside the mutex to determine
  // if waiting has terminated meaning it's safe to decrement the ref count. If
  // it's true (which we write with release semantics), then the mutex is always
  // entered.
...
#  ifdef WFMO
  ...
  event->RegisteredWaits.erase(
      std::remove_if(event->RegisteredWaits.begin(),
                     event->RegisteredWaits.end(), RemoveExpiredWaitHelper),
...
#  ifdef WFMO
    while (!event->RegisteredWaits.empty()) {
      ...
      // memory_order_relaxed: this is just an optimization to see if it is OK
      // to skip this waiter, and if it's observed to be false then it's OK to
      // bypass the mutex at that point.
      ...
      // We have to check `Waiter->StillWaiting` twice, once before locking as
      // an optimization to bypass the mutex altogether, and then again after
      // locking the WFMO mutex because we could have !waitAll and another event
      // could have ended the wait, in which case we must not unlock the same
      // waiter or else a SetEvent() call on an auto-reset event may end up with
      // a lost wakeup.
      if (!i->Waiter->StillWaiting.load(std::memory_order_relaxed)) {
        result = pthread_mutex_unlock(&i->Waiter->Mutex);
        assert(result == 0);

        // memory_order_seq_cst: Ensure this is run after the wfmo mutex is
        // unlocked
        int ref_count =
      ...
      if (i->Waiter->WaitAll) {
        ...
        // We technically should do i->Waiter->StillWaiting =
        // Waiter->Status.EventsLeft
        // != 0 but the only time it'll be equal to zero is if we're the last
        // event, so no one else will be checking the StillWaiting flag. We're
...
#  endif  // WFMO
          // memory_order_release: this is the synchronization point for any
          // threads spin-waiting for the event to become available.
    ...
    // memory_order_release: this is the synchronization point for any threads
    // spin-waiting for the event to become available.
    event->State.store(true, std::memory_order_release);
#  ifdef WFMO
    for (size_t i = 0; i < event->RegisteredWaits.size(); ++i) {
      ...
      // Waiter->StillWaiting may have become true by now, but we're just going
      // to pretend it hasn't. So long as we hold a reference to the WFMO, this
      // is safe since manual reset events are not one-time use.
...
int ResetEvent(neosmart_event_t event) {
  // memory_order_relaxed and no mutex: there can't be any guarantees about
  // concurrent calls to either of WFMO()/SetEvent() and ResetEvent() because
  // they're racy by nature. Only the behavior of concurrent WFMO() and
  // SetEvent() calls is strongly defined.
  event->State.store(false, std::memory_order_relaxed);
  return 0;
}
...
int PulseEvent(neosmart_event_t event) {
  // This may look like it's a horribly inefficient kludge with the sole
  // intention of reducing code duplication, but in reality this is what any
  // PulseEvent() implementation must look like. The only overhead (function
  // calls aside, which your compiler will likely optimize away, anyway), is if
  // only WFMO auto-reset waits are active there will be overhead to
  // unnecessarily obtain the event mutex for ResetEvent() after. In all other
  // cases (being no pending waits, WFMO manual-reset waits, or any WFSO waits),
  // the event mutex must first be released for the waiting thread to resume
  // action prior to locking the mutex again in order to set the event state to
  // unsignaled, or else the waiting threads will loop back into a wait (due to
  // checks for spurious CVariable wakeups).
...
namespace neosmart {
neosmart_event_t CreateEvent(bool manualReset, bool initialState) {
  return static_cast<neosmart_event_t>(
      ::CreateEvent(NULL, manualReset, initialState, NULL));
}
...
int WaitForEvent(neosmart_event_t event, uint64_t milliseconds) {
  ...
  // WaitForSingleObject(Ex) and WaitForMultipleObjects(Ex) only support 32-bit
  // timeout
  ...
  if (result == WAIT_OBJECT_0 || result == WAIT_ABANDONED) {
    // We must swallow WAIT_ABANDONED because there is no such equivalent on
    // *nix
    return 0;
  }

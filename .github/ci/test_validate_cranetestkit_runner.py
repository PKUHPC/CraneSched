...
class ReleaseValidationTest(unittest.TestCase):
    ...
    @unittest.skipUnless(os.geteuid() == 0, "release ownership contract requires root")
    def test_generated_python_bytecode_is_accepted(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            release, profile, _executable, revision, executable_sha256 = self._release(
                Path(temporary)
            )
            source_package = release / "src/crane_testkit"
            _make_writable(source_package)
            cache = source_package / "__pycache__"
            cache.mkdir()
            bytecode = cache / "__init__.cpython-312.pyc"
            bytecode.write_bytes(b"generated bytecode")
            source_package.chmod(
                stat.S_IMODE(source_package.lstat().st_mode) & ~0o222
            )
            self.assertNotEqual(cache.lstat().st_mode & 0o222, 0)
            self.assertNotEqual(bytecode.lstat().st_mode & 0o222, 0)
            validation._validate_release(
                release, revision, profile, executable_sha256
            )

    @unittest.skipUnless(os.geteuid() == 0, "release ownership contract requires root")
    def test_sourceless_python_bytecode_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            release, profile, _executable, revision, executable_sha256 = self._release(
                Path(temporary)
            )
            source_package = release / "src/crane_testkit"
            _make_writable(source_package)
            (source_package / "injected.pyc").write_bytes(b"sourceless bytecode")
            _freeze(release)
            with self.assertRaisesRegex(
                validation.ValidationError, "ignored files outside .venv"
            ):
